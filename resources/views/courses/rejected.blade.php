                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <select id="rejected_instructor_id" class="form-control select2">
                                        <option value="">{{ __('All') }}</option> @foreach ($instructors as $instructor) <option value="{{ $instructor->id }}">{{ $instructor->name }}</option> @endforeach </select>
                                </div>
                                @endif
...
@section('script')
    ...
    <script>
        $(document).ready(function(){
            ...
            $('#rejected_instructor_id').on('change', function(){
                $('#table_rejected').bootstrapTable('refresh');
            });
            $('#reset_rejected_filters').on('click', function(){
                $('#rejected_instructor_id').val(null).trigger('change');
                $('#table_rejected').bootstrapTable('refresh');
            });
        });
