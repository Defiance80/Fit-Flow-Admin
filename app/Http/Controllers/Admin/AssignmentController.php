...
class AssignmentController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->filled('instructor_id')) {
                $query->whereHas('assignment.chapter.course', function($courseQuery) use ($request) {
    ...
    public function statistics(Request $request)
    {
        try {
            ...
            $courseBreakdown = (clone $query)
                ->join('course_chapter_assignments', 'user_assignment_submissions.course_chapter_assignment_id', '=', 'course_chapter_assignments.id')
            ...
            $statistics = [
                'overview' => [
                    ...
                    'acceptance_rate' => $totalSubmissions > 0 ? round(($acceptedSubmissions / $totalSubmissions) * 100, 2) : 0,
                ],
    ...
    public function show($id)
    {
        try {
            ...
            if (!$submission->user) {
                return redirect()->route('admin.assignments.index')->with('error', 'User not found for this submission.');
            }
            
            if (!$submission->assignment) {
                return redirect()->route('admin.assignments.index')->with('error', 'Assignment not found for this submission.');
            }
            
            if (!$submission->assignment->chapter) {
                return redirect()->route('admin.assignments.index')->with('error', 'Chapter not found for this assignment.');
            }
            
            if (!$submission->assignment->chapter->course) {
                return redirect()->route('admin.assignments.index')->with('error', 'Course not found for this chapter.');
            }
            ...
            return redirect()->route('admin.assignments.index')->with('error', 'Failed to load assignment submission: ' . $e->getMessage());
        }
    }
    ...
    public function bulkUpdate(Request $request)
    {
        try {
            ...
            $updatedCount = UserAssignmentSubmission::whereIn('id', $submissionIds)
                ->update($updateData);

            return redirect()->back()->with('success', "Successfully updated {$updatedCount} assignment submissions");

        } catch (\Exception $e) {
            return redirect()->back()->with('error', 'Failed to bulk update assignment submissions: ' . $e->getMessage());
        }
    }
