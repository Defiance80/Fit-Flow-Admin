...
class UserController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $query = User::with('instructor_details')
            ->orderBy($sort, $order);
        ...
        foreach ($result as $row) {
            ...
            $operate .= '<a href="' . $viewUrl . '" class="btn icon btn-xs btn-rounded btn-icon rounded-pill btn-info view_btn" title="View" data-target="#userDetailsModal" data-toggle="modal" id="' . $row->id . '"><i class="fas fa-eye"></i></a>&nbsp;&nbsp;';
    ...
    public function toggleStatus(Request $request, $id)
    {
        try {
            ...
            DB::rollBack();
            return response()->json(['success' => false, 'message' => 'Failed to update user status: ' . $e->getMessage()], 500);
        }
    }
}
