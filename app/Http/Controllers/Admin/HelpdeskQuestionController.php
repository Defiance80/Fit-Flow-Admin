...
class HelpdeskQuestionController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            $query = HelpdeskQuestion::with(['group', 'user', 'replies'])
            ...
            foreach ($result as $row) {
                ...
                if ($showDeleted == 1 || $showDeleted === '1') {
                    ...
                    $operate = BootstrapTableService::restoreButton(route('admin.helpdesk.questions.restore', $row->id));
                    $operate .= BootstrapTableService::trashButton(route('admin.helpdesk.questions.trash', $row->id));
                } else {
                    // Active items - show view, edit, delete
                    $operate = BootstrapTableService::button('fa fa-eye', route('admin.helpdesk.questions.show', $row->id), ['btn-info', 'view-question'], ['title' => 'View']);
                    $operate .= BootstrapTableService::editButton(route('admin.helpdesk.questions.edit', $row->id), false, null, null, null, 'fa fa-edit');
                    $operate .= BootstrapTableService::deleteButton(route('admin.helpdesk.questions.destroy', $row->id), null, $row->id);
                }
                ...
                $tempRow = [
                    ...
                    'description' => (string) \Illuminate\Support\Str::limit($row->description, 50),
