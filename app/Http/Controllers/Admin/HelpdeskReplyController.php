...
class HelpdeskReplyController extends Controller
{
    public function index(Request $request)
    {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            foreach ($result as $row) {
                $tempRow = [
                    'id' => (string) $row->id,
                    'reply' => (string) \Illuminate\Support\Str::limit($row->reply, 100),
                    'question_title' => (string) ($row->question->title ?? 'N/A'),
                    'user_name' => (string) ($row->user->name ?? 'N/A'),
                    'parent_reply' => (string) ($row->parent ? \Illuminate\Support\Str::limit($row->parent->reply, 30) : 'N/A'),
    ...
    public function destroy($id)
    {
        try {
            ...
            return ResponseService::successResponse('Reply deleted successfully');
        } catch (Throwable $th) {
