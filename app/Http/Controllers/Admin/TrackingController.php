...
class TrackingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $baseQuery = UserCurriculumTracking::select('user_curriculum_trackings.user_id', 'course_chapters.course_id')
            ...
            ->whereHas('user.orders', function($q) {
        ...
        return view('pages.admin.tracking.index', compact('trackings', 'stats', 'courses', 'instructors'), ['type_menu' => 'tracking']);
    }
    ...
    public function show($id)
    {
        ...
        if (count($parts) !== 2) {
            return redirect()->route('admin.tracking.index')
                ->with('error', 'Invalid tracking ID format.');
        }
    ...
    {
        $data = [
            'progress_distribution' => OrderCourse::select(
                DB::raw('CASE 
    ...
    private function getDetailedProgressFromCurriculum($userId, $courseId)
    {
        ...
        $curriculumTrackings = UserCurriculumTracking::where('user_id', $userId)
        ...
        $lastCompleted = $curriculumTrackings->where('status', 'completed')
        ...
        $chaptersData = $chapters->map(function($chapter) use ($curriculumTrackings, $userId) {
    ...
    private function getDetailedProgress($tracking)
    {
        ...
        if (isset($tracking->user_id) && isset($tracking->course_id)) {
    ...
    private function updateCourseProgress($userId, $chapterId)
    {
        ...
        $courseTracking = UserCourseTrack::where('user_id', $userId)
    ...
    public function updateChapterProgressApi(Request $request)
    {
        ...
        try {
            $tracking = $this->updateChapterProgress(
                $request->user_id,
                $request->chapter_id,
                $request->status
            );
    ...
    private function getStatisticsFromCurriculum()
    {
        ...
        $userCoursePairs = UserCurriculumTracking::select('user_curriculum_trackings.user_id', 'course_chapters.course_id')
            ->join('course_chapters', 'user_curriculum_trackings.course_chapter_id', '=', 'course_chapters.id')
            ->join('users', 'user_curriculum_trackings.user_id', '=', 'users.id')
            ->whereHas('user.orders', function($q) {
                $q->where('status', 'completed');
