...
class CertificateController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax() || $request->wantsJson()) {
            ...
            foreach ($result as $row) {
                ...
                $operate = BootstrapTableService::button('fa fa-eye', route('admin.certificates.show', $row->id), ['btn-info'], ['title' => 'View']);
                $operate .= BootstrapTableService::editButton(route('admin.certificates.edit', $row->id), false, null, null, null, 'fa fa-edit');
                $operate .= BootstrapTableService::button('fa fa-search', route('admin.certificates.preview', $row->id), ['btn-secondary'], ['title' => 'Preview', 'target' => '_blank']);
                $operate .= BootstrapTableService::button('fa fa-edit', route('admin.certificates.editor', $row->id), ['btn-primary'], ['title' => 'Edit Design']);
                $operate .= BootstrapTableService::deleteButton(route('admin.certificates.destroy', $row->id), null, $row->id);
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'background_image' => [
                ...
                function ($attribute, $value, $fail) {
                    if ($value && !in_array(strtolower($value->getClientOriginalExtension()), ['jpeg', 'jpg', 'png', 'gif', 'svg', 'webp'])) {
        ...
        if ($existingCertificate) {
            ...
            if ($request->ajax() || $request->wantsJson()) {
                ResponseService::validationError("A certificate of type '{$typeDisplay}' already exists. Only one certificate per type is allowed.");
            }
            return redirect()->back()
                ->withErrors(['type' => "A certificate of type '{$typeDisplay}' already exists. Only one certificate per type is allowed."])
                ->withInput();
        }
