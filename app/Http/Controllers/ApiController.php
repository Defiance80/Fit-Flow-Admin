...
class ApiController extends Controller
{
        ...
        try {
            ...
            $userQuery = User::role(config('constants.SYSTEM_ROLES.GENERAL_USER'))
                ...
                ->when($request->has('mobile'), function($query) use ($request){
                    $query->where(['mobile' => $request->mobile, 'country_code' => $request->country_code]);
                });
    ...
    public function userSignup(Request $request)
    {
        try {
            ...
            $socialLogin = SocialLogin::where('firebase_id', $firebaseId)->where('type', $request->type)->with('user', function ($q) {
                $q->withTrashed();
    ...
    public function mobileLogin(Request $request)
    {
        try {
            ...
            $user = User::withTrashed()->role(config('constants.SYSTEM_ROLES.GENERAL_USER'))->where('mobile', $request->mobile)->first();
    ...
    public function mobileRegistration(Request $request)
    {
        try {
            ...
            $socialLogin = SocialLogin::where(['firebase_id' => $firebaseId, 'user_id' => $user->id])->where('type', 'phone')->first();
            if(empty($socialLogin)){
                SocialLogin::create([
                    'firebase_id' => $firebaseId,
                    'user_id' => $user->id,
    ...
    {
        try {
            $user = User::where(['id' => Auth::user()->id, 'is_active' => 1])
            ...
            $userData['wallet_balance'] = (float) ($user->wallet_balance ?? 0);
        ...
        try {
            ...
            if ($isInstructor) {
                ...
                if ($existingInstructor) {
                    $existingInstructorPersonalDetail = InstructorPersonalDetail::where('instructor_id', $existingInstructor->id)->first();
            ...
            if ($isInstructor) {
                ...
                $hasInstructorData = $request->has('instructor_type') || 
                ...
                if ($request->hasFile('team_logo')) {
                    ...
                    $personalDetailsData['team_logo'] = FileService::compressAndReplace($request->team_logo, $instructorPersonalDetailFolder, $existingFile);
                }
                if ($request->hasFile('id_proof')) {
                    $existingFile = !empty($instructorPersonalDetail) ? $instructorPersonalDetail->getRawOriginal('id_proof') : null;
                    $personalDetailsData['id_proof'] = FileService::compressAndReplace($request->id_proof, $instructorPersonalDetailFolder, $existingFile);
                }
                if ($request->hasFile('preview_video')) {
                    $existingFile = !empty($instructorPersonalDetail) ? $instructorPersonalDetail->getRawOriginal('preview_video') : null;
                    $personalDetailsData['preview_video'] = FileService::compressAndReplace($request->preview_video, $instructorPersonalDetailFolder, $existingFile);
                }
            ...
            $user = User::where(['id' => $user->id, 'is_active' => 1])
                ->with([
                    'instructor_details.personal_details',
                    'instructor_details.social_medias.social_media',
                    'instructor_details.other_details.custom_form_field',
    ...
    public function changePassword(Request $request)
    {
        try {
            ...
            if (empty($user->password)) {
                return ApiResponseService::validationError('You cannot change password. Please set a password first using forgot password.');
            }
            ...
            return ApiResponseService::successResponse('Password changed successfully. Please login again with your new password.');
        ...
        try {
            ...
            if ($request->filled('user_team_slug')) {
                ...
                if ($authenticatedUserInstructorId == $teamUserInstructorId) {
                    ...
                    if ($authenticatedUserTeam && $teamUserTeam && 
                ...
                if (!$isInSameTeam) {
                    return ApiResponseService::unauthorizedResponse('You are not authorized to view this team\'s notifications');
                }
            ...
            if ($type === 'all' || $type === 'global') {
                ...
                $globalNotifications = \App\Models\Notification::orderBy('date_sent', 'desc')
                    ->get()
                    ->map(function($notification) use ($readNotificationIds, $user) {
                        $slug = null;
                        
                        // Get slug for course or instructor notification types
                        if ($notification->type === 'course' && $notification->type_id) {
                            $course = \App\Models\Course\Course::find($notification->type_id);
                            $slug = $course->slug ?? null;
                        } elseif ($notification->type === 'instructor' && $notification->type_id) {
                            $instructor = \App\Models\Instructor::with('user')->find($notification->type_id);
                    ...
                    ->filter(function($notification) use ($status) {
                        // Apply status filter
                        if ($status === 'read') {
            ...
            if ($type === 'all' || $type === 'personal') {
                ...
                $allUserNotifications = DB::table('notifications')
                    ->where('notifiable_id', $user->id)
                    ->get();
                
                // Log for debugging
                \Illuminate\Support\Facades\Log::info('All notifications for user', [
                    'user_id' => $user->id,
                    'total_count' => $allUserNotifications->count(),
                    'notifications' => $allUserNotifications->map(function($n) {
                        return [
                            'id' => $n->id,
                            'type' => $n->type,
                ...
                $personalNotificationsQuery = DB::table('notifications')
                    ->where('notifiable_id', $user->id)
                    ->where('notifiable_type', 'App\Models\User'); // Direct match as shown in database
                ...
                // Log for debugging
                \Illuminate\Support\Facades\Log::info('Personal notifications query result', [
                    'user_id' => $user->id,
                    ...
                    'first_notification' => $personalNotificationsRaw->first() ? [
                        ...
                        'data_preview' => substr($personalNotificationsRaw->first()->data, 0, 100)
                    ] : null
                ]);
                
                $personalNotifications = $personalNotificationsRaw->map(function($notification) {
                        ...
                        if ($notificationType === 'course' && $typeId) {
                            $course = \App\Models\Course\Course::find($typeId);
                            $slug = $course->slug ?? null;
                        } elseif ($notificationType === 'instructor' && $typeId) {
                            $instructor = \App\Models\Instructor::with('user')->find($typeId);
                            $slug = $instructor->user->slug ?? null;
                        } elseif ($notificationType === 'team_invitation' && $typeId) {
                            // Get team member from type_id
                            $teamMember = \App\Models\TeamMember::with(['instructor.user', 'instructor.personal_details', 'instructor.social_medias'])->find($typeId);
                            ...
                            if ($teamMember && $teamMember->instructor) {
                                ...
                                $teamMembers = [
                                    ...
                                    'created_at' => $teamMember->created_at ? $teamMember->created_at->format('Y-m-d H:i:s') : null,
                                    'updated_at' => $teamMember->updated_at ? $teamMember->updated_at->format('Y-m-d H:i:s') : null,
                                ];
                            }
                        }
                        
                        // Parse created_at and read_at timestamps
                        $createdAt = $notification->created_at ? (is_string($notification->created_at) ? \Carbon\Carbon::parse($notification->created_at) : $notification->created_at) : now();
                        $readAt = $notification->read_at ? (is_string($notification->read_at) ? \Carbon\Carbon::parse($notification->read_at) : $notification->read_at) : null;
            ...
            \Illuminate\Support\Facades\Log::info('Before pagination', [
                'total_count' => $notifications->count(),
                'sample_notifications' => $notifications->take(3)->map(function($n) {
                    return [
                        'id' => $n['id'] ?? null,
                        'type' => $n['type'] ?? null,
                        'title' => $n['title'] ?? null,
            ...
            $globalUnreadCount = 0;
            if ($type === 'all' || $type === 'global') {
                // Get all global notification IDs
                $allGlobalNotificationIds = \App\Models\Notification::pluck('id')->toArray();
    ...
    public function markNotificationAsRead(Request $request)
    {
        try {
            ...
            if ($request->filled('user_team_slug')) {
                ...
                if (!$isInSameTeam) {
                    return ApiResponseService::unauthorizedResponse('You are not authorized to mark this team\'s notifications as read');
                }
                        ...
                        if (!$existingRead) {
                            ...
                            try {
                                ...
                                if (str_contains($e->getMessage(), 'Duplicate entry') || str_contains($e->getMessage(), 'UNIQUE constraint')) {
                                    ...
                                    $errors[] = "Failed to mark notification {$notificationIdInt} as read: " . $e->getMessage();
                                }
                            }
                        } else {
    ...
    public function deleteAccount(Request $request)
    {
        try {
            ...
            if (!in_array($confirmDeletion, [true, 1, '1', 'true', 'yes'], true)) {
                return ApiResponseService::validationError('You must confirm that you agree to permanently delete your account and all associated data.');
            }
            ...
            if (in_array($userType, ['email', 'mobile'])) {
                ...
                if (!Hash::check($password, $user->password)) {
                    return ApiResponseService::validationError('Incorrect password. Please enter your current password to confirm account deletion.');
                }
                ...
                if (empty($request->firebase_token)) {
                    return ApiResponseService::validationError('Firebase token is required to confirm account deletion.');
                }
                ...
                try {
                    ...
                    if (empty($socialLogin)) {
                        return ApiResponseService::validationError('Invalid firebase token. Please provide a valid token to confirm account deletion.');
                    }
                ...
                if (!empty($user->password)) {
                    ...
                    return ApiResponseService::validationError('Unable to verify account deletion. Please contact support.');
                }
            }
            ...
            try {
                ...
                \App\Models\Course\CourseChapter\Assignment\UserAssignmentSubmission::where('user_id', $user->id)->delete();
                ...
                return ApiResponseService::successResponse('Your account has been successfully deleted. All your data has been removed from our system.');
            ...
            return ApiResponseService::errorResponse('Failed to delete account. Please try again later.', exception: $th);
        }
    }
        ...
        try {
            ...
            try {
                ...
                Mail::send('emails.contact-form', [
                    ...
                    'appName' => $appName
                ], function ($message) use ($adminEmail, $appName, $contactMessage) {
                    $message->to($adminEmail)
                            ->subject('New Contact Form Submission - ' . $appName)
                            ->replyTo($contactMessage->email, $contactMessage->first_name);
                });
            } catch (\Exception $e) {
            ...
            return ApiResponseService::successResponse('Your message has been sent successfully! We will get back to you soon.');
    ...
    public function getCategories(Request $request)
    {
        try {
            ...
            $categoryQuery = Category::select('id', 'name', 'image', 'parent_category_id', 'description', 'status', 'slug', 'sequence')
                        ->withCount(['subcategories' => function ($q) {
                            $q->where('status', 1);
                        }])
                        ->withCount(['parent_category' => function ($q) {
                            $q->where('status', 1);
                        }])
                            ...
                            if($request->has('id')){
                                ...
                                if($request->has('get_subcategory') && $request->get_subcategory == 1){
                                    $query->with(['subcategories' => function($subQuery) {
                                        $subQuery->orderByRaw('CASE WHEN sequence IS NULL THEN 1 ELSE 0 END')
    ...
    public function getSubCategories(Request $request)
    {
        ...
        try {
            ...
            $categoryQuery = Category::select('id', 'name', 'image', 'parent_category_id', 'description', 'status', 'slug')
                ->withCount(['subcategories' => function ($q) {
                ...
                ->with([
                    ...
                    'subcategories' => function ($query) {
                        $query->select('id', 'sequence', 'name', 'image', 'parent_category_id', 'description', 'status', 'slug')
                            ->where('status', 1)
                            ->orderBy('sequence', 'ASC')
                            ->withCount(['subcategories' => function ($q) {
                                $q->where('status', 1);
    ...
    public function getCustomFormFields(Request $request)
    {
        try {
            $customFormFields = CustomFormField::select('id', 'name', 'type', 'is_required', 'sort_order')
                ->with('options:id,custom_form_field_id,option')
                ->orderBy('sort_order')
                ->get();
    ...
    public function getAppSettings(Request $request)
    {
        try {
            ...
            $appSettings = HelperService::systemSettings(array('playstore_url', 'appstore_url', 'android_version', 'ios_version', 'app_version', 'maintaince_mode', 'force_update', 'app_name', 'website_url', 'announcement_bar', 'favicon', 'vertical_logo', 'horizontal_logo', 'placeholder_image', 'contact_address', 'contact_email', 'contact_phone'));
            ...
            $defaultLanguage = Language::where('status', 1)
    ...
    public function getWebSettings(Request $request)
    {
        try {
            $generalSystemSettings = ApiService::getGeneralSystemSettings();
            $webSettings = HelperService::systemSettings(array('individual_instructor_terms', 'team_instructor_terms', 'app_name', 'website_url', 'announcement_bar', 'playstore_url', 'appstore_url', 'favicon', 'vertical_logo', 'horizontal_logo', 'placeholder_image', 'contact_address', 'contact_email', 'contact_phone', 'schema', 'system_light_colour', 'maintaince_mode', 'hover_color', 'footer_description'));
            ...
            $webSettings = array_merge($generalSystemSettings, $webSettings, array('social_media' => $socialMedia));
    ...
    public function getPaymentIntent(Request $request) {
        ...
        try {
            ...
            $purchasedCourse = UserCourseTrack::where(['user_id' => Auth::user()->id, 'course_id' => $request->course_id])->first();
            ...
            $paymentIntent = PaymentService::create($paymentSettings)->createAndFormatPaymentIntent(round($course->price, 2), [
    ...
    public function getSystemLanguages(Request $request)
    {
        ...
        if ($code) {
            ...
            if ($systemType === 'app') {
                ...
                $langData['translations_app'] = File::exists($file_app)
            ...
            elseif ($systemType === 'web') {
                ...
                $langData['translations_web'] = File::exists($file_web)
        ...
        $defaultLang = Language::where('status', 1)->where('is_default', true)->first()
            ?? Language::where('status', 1)->where('code', 'en')->first()
            ?? $languages->first();
    
        // 🔹 Prepare languages list with empty translations
        $formattedLanguages = $languages->map(function ($language) use ($systemType) {
            $lang = [
                'id' => $language->id,
                'name' => $language->name,
                'code' => $language->code,
    ...
    public function getPages(Request $request)
    {
        try {
            ...
            $pagesQuery = Page::where('status', 1) // Only active pages
                ->with('language');
            ...
            $pages = $pagesQuery->orderBy('id', 'asc')
                ->get()
                ->map(function ($page) {
                    ...
                    $pageTypeSlug = $pageTypeSlugMap[$page->page_type] ?? strtolower(str_replace(' ', '-', $page->page_type));
    ...
    public function getSeoSettings(Request $request)
    {
        try {
            ...
            $seoSettings = $seoSettingsQuery->orderBy('id', 'asc')
                ->get()
                ->map(function ($seoSetting) {
                    return [
                        'id' => $seoSetting->id,
                        ...
                        'og_image' => $seoSetting->og_image ? url(\Illuminate\Support\Facades\Storage::url($seoSetting->og_image)) : null,
