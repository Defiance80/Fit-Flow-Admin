...
class HelpdeskApiController extends Controller
{
    ...
    public function checkGroupApproval(Request $request)
    {
        try {
            ...
            if ($request->filled('group_id')) {
                $group = HelpdeskGroup::where('id', $request->group_id)
                    ->where('is_active', true)
                    ->first();
            } else {
                $group = HelpdeskGroup::where('slug', $request->group_slug)
                    ->where('is_active', true)
                    ->first();
            }
            ...
            $userRequest = HelpdeskGroupRequest::where('user_id', $userId)
                ->where('group_id', $group->id)
                ->first();
    ...
    public function questions(Request $request)
    {
        try {
            ...
            if ($request->filled('group_id') || $request->filled('group_slug')) {
                ...
                $totalReplies = HelpdeskQuestion::whereHas('group', function($q) use ($request) {
                    if ($request->filled('group_id')) {
                        $q->where('id', $request->group_id);
                    } else {
                        $q->where('slug', $request->group_slug);
                    }
                })->withCount('replies')->get()->sum('replies_count');
            } else {
    ...
    public function showQuestion(Request $request)
    {
        try {
            ...
            if ($request->filled('slug')) {
                $question = HelpdeskQuestion::with(['user', 'group'])
    ...
    public function __construct()
    {
        ...
        $this->middleware('auth:sanctum')->only([
            'storeGroup', 'requestJoin', 'approveRequest', 'rejectRequest',
            'storeQuestion', 'storeReply', 'checkGroupApproval'
        ]);
    }
   ...
   public function storeQuestion(Request $request)
{
    ...
    public function storeReply(Request $request)
    {
        try {
            ...
            $reply = HelpdeskReply::with(['user', 'children.user'])
                ->find($reply->id);
            ...
            $reply->replies = $reply->children->map(function ($nestedReply) {
    ...
    public function search(Request $request)
    {
        try {
            ...
            if ($type === 'questions' || $type === 'all') {
                ...
                $results['questions']['data'] = $questions->map(function($question) {
                    return [
                        'id' => $question->id,
                        'title' => $question->title,
                        'slug' => $question->slug,
            ...
            if ($type === 'groups' || $type === 'all') {
                ...
                $results['groups']['data'] = $groups->map(function($group) {
                    return [
                        'id' => $group->id,
                        'name' => $group->name,
                        'slug' => $group->slug,
