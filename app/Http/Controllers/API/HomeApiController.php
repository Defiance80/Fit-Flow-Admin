...
class HomeApiController extends Controller
{
    ...
    public function getCounts(Request $request)
    {
        ...
        // Count active courses that have at least one active chapter
            ...
            $studentEnrollCount = User::whereHas('orders', function ($query) {
                $query->where('status', 'completed')
                    ->whereHas('orderCourses');
            })->distinct()->count();
    ...
    public function getCategoriesWithCourseCount()
    {
        try {
            ...
            ->withCount([
                'courses as active_course_count' => function ($query) {
            ...
            return ApiResponseService::successResponse('Categories with active course count retrieved successfully.', $categories);
        } catch (Throwable $e) {
    ...
    public function getFeatureSections(Request $request)
    {
        ...
        $sections = FeatureSection::where('is_active', 1)
            ->orderBy('row_order')
            ->get();
        ...
        $result = $sections->map(function ($section) use ($user, $request) {
            ...
            switch ($section->type) {
                ...
                case 'top_rated_courses':
                    ...
                    $data = $query->orderByDesc('ratings_avg_rating')
                        ->take($limit)
                        ->get();
                    break;
                ...
                case 'most_viewed_courses':
                    ...
                    $data = $query->orderByDesc('views_count')
                ...
                case 'offer':
                    $data = $section->images()
                        ->when($section->limit, fn($q) => $q->take($section->limit))
                        ->get();
                    break;
                ...
                case 'top_rated_instructors':
                    ...
                    $data = $instructors->map(function ($instructor) {
                        ...
                        $activeCoursesCount = Course::where('user_id', $instructor->user_id)
                            ...
                            ->count();
                        $publishedCoursesCount = Course::where('user_id', $instructor->user_id)->where('is_active', 1)->where('approval_status', 'approved')->count();
                ...
                case 'my_learning':
                    if ($user) {
                        ...
                        if ($request->filled('course_type')) {
                            ...
                            $enrolledCourses = $enrolledCourses->filter(function($course) use ($courseTypes) {
                                return $course && in_array($course->course_type, $courseTypes);
                            })->values();
                        }
                        ...
                        $data = $enrolledCourses->map(function ($course) use ($user) {
                            if (!$course) {
                                return null;
                            }
                            ...
                            return $course['last_learned_at'] ?? $course['enrolled_at'] ?? '1970-01-01 00:00:00';
                        })->values()
                        ->take($limit);
                    } else {
                        $data = collect();
            ...
            if (in_array($section->type, ['newly_added_courses', 'top_rated_courses', 'free_courses', 'wishlist', 'recommend_for_you', 'searching_based', 'most_viewed_courses'])) {
                $data = $data->map(function ($course) use ($user) {
                    $discountPercentage = 0;
                    ...
                    $isWishlisted = Auth::check()
                        ? $course->wishlistedByUsers->contains('id', Auth::id())
                        : false;

                    $isEnrolled = Auth::check()
                        ? \App\Models\OrderCourse::whereHas('order', function($q) {
                    ...
                    return [
                        ...
                        'discount_price'    => $course->display_discount_price,
                        'discount_percentage' => $discountPercentage,
                        'is_wishlisted'       => $isWishlisted,
                        'is_enrolled'         => $isEnrolled,
                    ];
