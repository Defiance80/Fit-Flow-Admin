...
class CourseChapterApiController extends Controller {
    ...
    public function getCourseChapters(Request $request)
    {
        try {
                ...
                ->with([
                    ...
                    'lectures' => function($lectureQuery) {
                        $lectureQuery->where('is_active', true)
                            ...
                            ->with(['resources' => function($resourceQuery) {
                                $resourceQuery->where('is_active', true)
                                    ->orderBy('order');
                            }]);
                    },
            ...
            $formattedChapters = $chapters->map(function($chapter) use ($isItemCompleted, $user) {
    ...
    public function trackCourseChapter(Request $request)
    {
        ...
        try {
            ...
            if ($hasApprovedRefund) {
                return ApiResponseService::validationError('Refund is approved for this course. Progress tracking is disabled.');
            }
    ...
    public function getCurriculumProgress(Request $request)
    {
        ...
        try {
            ...
            foreach ($chapters as $chapter) {
                ...
                $totalContent = $chapter->lectures->count() + $chapter->quizzes->count() + 
                               $chapter->assignments->count() + $chapter->resources->count();
    ...
    public function getChapterCurriculumDetails(Request $request)
    {
        ...
        try {
            ...
            ApiResponseService::logErrorResponse($e, "API CourseChapterController -> getChapterCurriculumDetails Method");
    ...
    public function markCurriculumItemCompleted(Request $request)
    {
        ...
        try {
            ...
            $chapter = CourseChapter::with(['lectures', 'quizzes', 'assignments', 'resources', 'course'])
                ->find($chapterId);
            if (!$chapter) {
                return ApiResponseService::validationError('Chapter not found');
            }
            ...
            $formatFullCurriculumItem = function($item, $type, $chapter, $userId) {
                $baseData = [
                    'id' => $item->id,
                    'type' => $type,
                    'title' => $item->title,
                ...
                switch ($type) {
                    case 'lecture':
                        $baseData['lecture_type'] = $item->lecture_type ?? null;
                        $baseData['file'] = $item->file ?? null;
                        $baseData['youtube_url'] = $item->youtube_url ?? null;
                        $baseData['duration'] = ($item->hours ?? 0) * 3600 + ($item->minutes ?? 0) * 60 + ($item->seconds ?? 0);
                        $baseData['hours'] = $item->hours ?? 0;
                        $baseData['minutes'] = $item->minutes ?? 0;
                        $baseData['seconds'] = $item->seconds ?? 0;
                        $baseData['free_preview'] = $item->free_preview ?? false;
                        ...
                        $baseData['resources'] = $item->resources->map(function($resource) {
                            return [
                                'id' => $resource->id,
                                'title' => $resource->title,
                                'type' => $resource->type,
            ...
            ApiResponseService::logErrorResponse($e, "API CourseChapterController -> markCurriculumItemCompleted Method");
    ...
    public function getDetailedCurriculumTracking(Request $request)
    {
        ...
        try {
            $userId = Auth::user()->id;
            $query = UserCurriculumTracking::where('user_id', $userId)
                ->with(['chapter', 'user']);
            ...
            $groupedData = $trackingData->groupBy('course_chapter_id')->map(function($items, $chapterId) {
                ...
                return [
                    ...
                    'completion_percentage' => $items->count() > 0 ? 
                        round(($items->where('status', 'completed')->count() / $items->count()) * 100, 2) : 0,
            ...
            $response = [
                ...
                'overall_completion_percentage' => $trackingData->count() > 0 ? 
                    round(($trackingData->where('status', 'completed')->count() / $trackingData->count()) * 100, 2) : 0,
                'chapters' => $groupedData->values(),
            ];

            return ApiResponseService::successResponse('Detailed curriculum tracking retrieved successfully', $response);

        } catch (\Throwable $e) {
            ApiResponseService::logErrorResponse($e, "API CourseChapterController -> getDetailedCurriculumTracking Method");
        ...
        try {
            ...
            $response = [
                ...
                'course_title' => $currentCurriculum->chapter && $currentCurriculum->chapter->course ? 
                    $currentCurriculum->chapter->course->title : 'Unknown Course',
                ...
                'completed_at_formatted' => $currentCurriculum->completed_at ? 
                    $currentCurriculum->completed_at->format('Y-m-d H:i:s') : null,
    ...
    public function getAssignmentSubmissionHistory(Request $request)
    {
        try {
            ...
            $allAssignmentsQuery = \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::with(['chapter.course'])
            ...
            $chapters = $allAssignmentsByChapter->map(function($chapterAssignments, $chapterId) use ($submissionsByAssignment) {
                $firstAssignment = $chapterAssignments->first();
                $chapter = $firstAssignment->chapter;
                
                $formattedAssignments = $chapterAssignments->map(function($assignment) use ($submissionsByAssignment) {
                    $assignmentSubmissions = $submissionsByAssignment->get($assignment->id, collect());
                    
                    // Format submissions for this assignment
                    $formattedSubmissions = $assignmentSubmissions->map(function($submission) {
                        ...
                        if ($firstFile) {
                            if ($firstFile->type === 'file' && $firstFile->file) {
                                ...
                                $fileName = basename($firstFile->file);
                        ...
                        return [
                            ...
                            'files' => $submission->files->map(function($file) {
                                return [
                                    'id' => $file->id,
                                    'type' => $file->type,
                                    'file' => $file->type === 'file' && $file->file ? FileService::getFileUrl($file->file) : null,
                                    'file_name' => $file->type === 'file' && $file->file ? basename($file->file) : null,
                ...
                return [
                    ...
                    'course_image' => $chapter->course->thumbnail ? (str_starts_with($chapter->course->thumbnail, 'http') ? $chapter->course->thumbnail : asset('storage/' . $chapter->course->thumbnail)) : null,
            ...
            if ($chapterId) {
                ...
                $currentChapterAssignments = $filteredAssignmentsByChapter->map(function($chapterAssignments, $chapterId) use ($submissionsByAssignment) {
                    $firstAssignment = $chapterAssignments->first();
                    $chapter = $firstAssignment->chapter;
            ...
            return ApiResponseService::successResponse('Course assignment submission history retrieved successfully', $responseData);

        } catch (\Throwable $e) {
            ApiResponseService::logErrorResponse($e, "API CourseChapterController -> getAssignmentSubmissionHistory Method");
            return ApiResponseService::errorResponse('Failed to retrieve assignment submission history.'.$e->getMessage());
        }
    }
    ...
    public function checkCourseCompletion(Request $request)
    {
        try {
            ...
            // Check assignment submissions (must be submitted or accepted)
            $assignmentIds = [];
            foreach ($course->chapters as $chapter) {
                foreach ($chapter->assignments as $assignment) {
            ...
            $certificateStatus = 'free'; // Default for paid courses
            ...
            if ($isFreeCourse) {
                ...
                $certificateFee = $course->certificate_fee ?? 0;
