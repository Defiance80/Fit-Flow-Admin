...
use App\Rules\ValidResourceFile;

class CourseApiController extends Controller
{
    private string $uploadFolder;
    private string $videoUploadFolder;
    private string $metaImageUploadFolder;
    ...
    public function getCourses(Request $request)
    {
        ...
        if ($request->filled('instructor_slug')) {
            ...
            $query->whereHas('user', function ($q) use ($instructorSlug) {
        ...
        if ($request->filled('search')) {
            ...
            \App\Models\SearchHistory::recordSearch($search, $userId, $ipAddress);
            
            $query->where(function ($q) use ($search) {
                $q->where('title', 'LIKE', "%{$search}%")
                    ->orWhere('short_description', 'LIKE', "%{$search}%")
                    ->orWhere('level', 'LIKE', "%{$search}%")
                    ->orWhereHas('language', function ($langQuery) use ($search) {
        ...
        if ($featureSection) {
            ...
            switch ($featureSection->type) {
                ...
                case 'top_rated_courses':
                    ...
                    $query->having('ratings_avg_rating', '>=', 4)
                ...
                case 'recommend_for_you':
                    if ($user) {
                        ...
                        $purchasedCourseIds = \App\Models\OrderCourse::whereHas('order', function($q) use ($user) {
                        ...
                        if (!empty($searchHistories)) {
                            $searchBasedCourseIds = Course::where('is_active', 1)
                                ->where(function($q) use ($searchHistories) {
                        ...
                        if (!empty($recommendedCourseIds)) {
                            $query->whereIn('id', $recommendedCourseIds)
                                  ->inRandomOrder();
                        } else {
                ...
                case 'searching_based':
                    if ($user) {
                        ...
                        if (!empty($searchHistories)) {
                            ...
                            $query->where(function($q) use ($searchHistories) {
                                foreach ($searchHistories as $searchQuery) {
                                    $q->orWhere('title', 'LIKE', "%{$searchQuery}%")
                                      ->orWhere('short_description', 'LIKE', "%{$searchQuery}%")
                                      ->orWhereHas('category', function($catQuery) use ($searchQuery) {
                ...
                case 'my_learning':
                    if ($user) {
                        $enrolledCourseIds = \App\Models\OrderCourse::whereHas('order', function($q) use ($user) {
                            $q->where('user_id', $user->id)
                              ->where('status', 'completed');
                        })->pluck('course_id')->toArray();
        ...
        if ($request->filled('post_filter')) {
            if ($request->post_filter == 'newest') {
                ...
                $query->withCount(['orderCourses' => function($q) {
        ...
        if ($needsPostFiltering) {
            ...
            if ($request->filled('rating_filter')) {
                ...
                $allCourses = $allCourses->filter(function($course) use ($ratingFilters) {
            ...
            if ($request->filled('duration_filter')) {
                ...
                $allCourses = $allCourses->filter(function($course) use ($durationFilters) {
                    // Calculate total course duration
                    $totalDuration = 0;
                    foreach ($course->chapters as $chapter) {
                        foreach ($chapter->lectures as $lecture) {
                            $totalDuration += ($lecture->hours ?? 0) * 3600 + ($lecture->minutes ?? 0) * 60 + ($lecture->seconds ?? 0);
                        }
                    }
            ...
            if ($request->filled('post_filter') && $request->post_filter == 'most_popular') {
                $allCourses = $allCourses->sortByDesc(function($course) {
                    // Use order_courses_count if available (from withCount), otherwise count manually
                    if (isset($course->order_courses_count)) {
                        return $course->order_courses_count;
                    }
                    // Fallback: count enrollments for this course
                    return \App\Models\OrderCourse::where('course_id', $course->id)
        ...
        $courses->getCollection()->transform(function ($course) {
            ...
            $isEnrolled = Auth::check()
                ? \App\Models\OrderCourse::whereHas('order', function($query) {
            ...
            return [
                ...
                'tax_type' => HelperService::systemSettings('tax_type') ?? 'exclusive',
    ...
    public function getCourse(Request $request)
    {
        try {
            ...
            $courseQuery = \App\Models\Course\Course::with([
                'category',
            ...
            if ($user) {
                ...
                $latestOrderCourse = OrderCourse::whereHas('order', function ($q) use ($user) {
                    $q->where('user_id', $user->id)
                      ->where('status', 'completed');
                })
                ->where('course_id', $course->id)
                ...
                $isWishlist = \App\Models\Wishlist::where('user_id', $user->id)
                    ->where('course_id', $course->id)
                    ->exists();
            }
            ...
            if ($user) {
                ...
                $userCurriculumTracking = \App\Models\UserCurriculumTracking::where('user_id', $user->id)
                    ->whereIn('course_chapter_id', $chapterIds)
                    ->get()
                    ->groupBy(function ($item) {
                        return $item->course_chapter_id . '_' . $item->model_type . '_' . $item->model_id;
                    });
            }

            // Helper function to check if curriculum item is completed
            $isItemCompleted = function($chapterId, $modelType, $modelId) use ($userCurriculumTracking) {
                if (empty($userCurriculumTracking)) return false;
                $key = $chapterId . '_' . $modelType . '_' . $modelId;
                return isset($userCurriculumTracking[$key]) && $userCurriculumTracking[$key]->first()->status === 'completed';
            };
            ...
            foreach ($course->chapters as $chapter) {
                ...
                foreach ($chapter->lectures as $lecture) {
                    ...
                    $lectureDuration = ($lecture->hours ?? 0) * 3600 + ($lecture->minutes ?? 0) * 60 + ($lecture->seconds ?? 0);
                ...
                // Add lectures
                $lectures = $chapter->lectures->map(function($lecture) use ($chapter, $isItemCompleted) {
                    return [
                        'id' => $lecture->id,
                        'type' => 'lecture',
                        'title' => $lecture->title,
                        ...
                        'duration' => ($lecture->hours ?? 0) * 3600 + ($lecture->minutes ?? 0) * 60 + ($lecture->seconds ?? 0),
                        'hours' => $lecture->hours ?? 0,
                        'minutes' => $lecture->minutes ?? 0,
                        'seconds' => $lecture->seconds ?? 0,
                        'duration_formatted' => $this->formatDuration(($lecture->hours ?? 0) * 3600 + ($lecture->minutes ?? 0) * 60 + ($lecture->seconds ?? 0)),
                        'free_preview' => $lecture->free_preview ?? false,
                        'is_active' => $lecture->is_active,
                        'chapter_order' => $lecture->chapter_order,
                        'is_completed' => $isItemCompleted($chapter->id, \App\Models\Course\CourseChapter\Lecture\CourseChapterLecture::class, $lecture->id),
                        'has_resources' => $lecture->resources->count() > 0,
                        'resources' => $lecture->resources->map(function($resource) {
                            return [
                                'id' => $resource->id,
                ...
                // Add quizzes
                $quizzes = $chapter->quizzes->map(function($quiz) use ($chapter, $isItemCompleted) {
                    return [
                        'id' => $quiz->id,
                        'type' => 'quiz',
                        'title' => $quiz->title,
                        ...
                        'is_completed' => $isItemCompleted($chapter->id, \App\Models\Course\CourseChapter\Quiz\CourseChapterQuiz::class, $quiz->id),
                ...
                // Add assignments
                $assignments = $chapter->assignments->map(function($assignment) use ($chapter, $isItemCompleted, $user) {
                    ...
                    return [
                        ...
                        'is_completed' => $isItemCompleted($chapter->id, \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::class, $assignment->id),
                ...
                $resources = $chapter->resources->map(function($resource) use ($chapter, $isItemCompleted) {
                    return [
                        'id' => $resource->id,
                        'type' => 'document',
                        'title' => $resource->title,
                        ...
                        'is_completed' => $isItemCompleted($chapter->id, \App\Models\Course\CourseChapter\Resource\CourseChapterResource::class, $resource->id),
            ...
            $reviews = $course->ratings->map(function ($rating) {
                return [
                    'id' => $rating->id,
            ...
            $totalLectureCount = $course->chapters->where('is_active', 1)->sum(function ($chapter) {
                return $chapter->lectures->where('is_active', 1)->count();
            });

            // Calculate total curriculum count (lectures + quizzes + assignments + resources) - only active chapters and active items
            $totalCurriculumCount = $course->chapters->where('is_active', 1)->sum(function ($chapter) {
            ...
            if ($user) {
                ...
                $completedCurriculumCount = \App\Models\UserCurriculumTracking::where('user_id', $user->id)
                    ->whereIn('course_chapter_id', $chapterIds)
                    ->where('status', 'completed')
                    ->count();
            ...
            if ($course->user) {
                ...
                if ($course->user->instructor_details) {
                    ...
                    if ($instructorTypeValue === 'team') {
                        ...
                        $instructor = \App\Models\Instructor::where('user_id', $course->user->id)->first();
                ...
                $instructorDetails = [
                    ...
                    'social_media' => $course->user->instructor_details ? $course->user->instructor_details->social_medias->mapWithKeys(function($socialMedia) {
                        return [$socialMedia->title => $socialMedia->url];
                    }) : null,
                    'reviews' => $course->user->instructor_details ? [
                        'total_reviews' => $course->user->instructor_details->ratings->count(),
                        'average_rating' => round($course->user->instructor_details->ratings->avg('rating') ?? 0, 2),
                        'reviews_list' => $course->user->instructor_details->ratings->map(function($rating) {
                            return [
                                'id' => $rating->id,
                                'rating' => $rating->rating,
                                'review' => $rating->review,
            ...
            $response = [
                ...
                'enroll_students' => OrderCourse::whereHas('order', function ($q) {
                ...
                'preview_videos' => $this->getPreviewVideos($course),
            ];
            ...
            if ($user) {
                ...
                $currentCurriculum = \App\Models\UserCurriculumTracking::where('user_id', $user->id)
                    ->where('status', 'completed')
                        ->whereIn('course_chapter_id', $chapterIds)
                ...
                if ($currentCurriculum) {
                    ...
                    $response['current_curriculum'] = [
                        ...
                        'completed_at_human' => $currentCurriculum->completed_at ? 
                            $currentCurriculum->completed_at->diffForHumans() : null,
                    ];
                    } else {
     ...
     */
    private function getPreviewVideos($course)
    {
        $previewVideos = [];
        ...
        foreach ($course->chapters as $chapter) {
            foreach ($chapter->lectures as $lecture) {
                ...
                // Check different video sources
                if ($lecture->file && (str_contains($lecture->file, '.mp4') || str_contains($lecture->file, '.mov') || str_contains($lecture->file, '.avi'))) {
                    $videoUrl = asset('storage/' . $lecture->file);
                } elseif ($lecture->youtube_url) {
                    $videoUrl = $lecture->youtube_url;
                }

                // If we found a video, add it to preview videos
                if ($videoUrl) {
                    $previewVideos[] = [
                        'title' => $lecture->title,
                        'thumbnail' => $course->thumbnail,
    ...
    public function courseView(Request $request)
    {
        try {
            ...
            if ($userId) {
                // Get latest completed order for this course
                $latestOrderCourse = \App\Models\OrderCourse::whereHas('order', function ($q) use ($userId) {
            ...
            $response = [
                'course' => [
                    ...
                    'tags' => $course->tags ?? [],
                    'language' => $course->language->name ?? null,
                    'instructors' => $course->instructors ? $course->instructors->map(function($instructor) {
                        return [
                            'id' => $instructor->id,
        ...
        try {
            ...
            if ($request->filled('user_team_slug')) {
                ...
                if ($authenticatedUserInstructorId == $teamUserInstructorId) {
                    ...
                    $isTeamMember = \App\Models\TeamMember::where('instructor_id', $teamUserInstructorId)
                        ->where('user_id', $user->id)
                        ->exists();
                    if ($isTeamMember) {
                        $isInSameTeam = true;
                    ...
                    if (!$isInSameTeam) {
                        $isTeamMember = \App\Models\TeamMember::where('instructor_id', $authenticatedUserInstructorId)
                ...
                if (!$isInSameTeam) {
                    return ApiResponseService::validationError('You are not authorized to access this course. You are not in the same team.');
                }
            }
            ...
            if ($user) {
                ...
                if ($course->user_id == $user->id) {
                    ...
                    if ($instructorId) {
                        $isTeamMember = \App\Models\TeamMember::where('instructor_id', $instructorId)
            ...
            $response = [
                ...
                'learnings' => $course->learnings->map(function($learning) {
                    return [
                        'id' => $learning->id,
                        'title' => $learning->title
                    ];
                }),
                'requirements' => $course->requirements->map(function($requirement) {
                    return [
                        'id' => $requirement->id,
                ...
                'co_instructors' => $course->instructors->map(function($instructor) {
                    return [
                        'id' => $instructor->id,
                        'name' => $instructor->name,
                        'email' => $instructor->email,
                        ...
                        'years_of_experience' => $instructor->instructor_details->personal_details->years_of_experience ?? 0,
                        ...
                        'social_medias' => $instructor->instructor_details && $instructor->instructor_details->social_medias 
                            ? $instructor->instructor_details->social_medias->map(function($social) {
                                return [
                                    'title' => $social->title ?? '',
                                    'url' => $social->url ?? '',
                                ];
                ...
                'chapters' => $course->chapters->map(function($chapter) {
                    ...
                    $isItemCompleted = function($chapterId, $modelType, $modelId) use ($user) {
                    ...
                    $quizzes = $chapter->quizzes->map(function($quiz) use ($chapter, $isItemCompleted) {
                        return [
                            ...
                            'resources' => $quiz->resources->map(function($resource) {
                                return [
                                    'id' => $resource->id,
                                    'title' => $resource->title,
                                    'file' => $resource->file,
                    ...
                    $chapterResources = $chapter->resources->map(function($resource) use ($chapter) {
                        return [
                            'id' => $resource->id,
                            'type' => 'resource',
                            'title' => $resource->title,
            ...
            if ($request->filled('student_enrolled') && $request->boolean('student_enrolled')) {
                ...
                $finalResponse['student_enrolled'] = $enrolledStudents->map(function($student) {
                    return [
                        'id' => $student->id,
                        'name' => $student->name,
                        'email' => $student->email,
        ...
        try {
            ...
            if ($request->filled('team_user_id') || $request->filled('team_user_slug')) {
                ...
                if ($request->filled('team_user_id')) {
                    $teamUser = \App\Models\User::find($request->team_user_id);
                } elseif ($request->filled('team_user_slug')) {
                    $teamUser = \App\Models\User::where('slug', $request->team_user_slug)->first();
                }
                ...
                if ($teamUser->id === $authUser->id) {
                    ...
                    if ($authInstructorDetails) {
                        $isTeamMember = \App\Models\TeamMember::where('instructor_id', $authInstructorDetails->id)
                            ->where('user_id', $teamUser->id)
                            ->where('status', 'approved')
                            ->exists();
                    }
                    ...
                    if ($teamUserInstructorDetails) {
                        $isInvitor = \App\Models\TeamMember::where('instructor_id', $teamUserInstructorDetails->id)
                ...
                \Illuminate\Support\Facades\Log::info('Team user courses query', [
            ...
            if ($isSelfAssignedCoursesRequest && $teamUser) {
                ...
                $query = Course::where('user_id', $targetUserId)
                    ->with(['category', 'chapters.lectures', 'chapters.quizzes', 'chapters.assignments']);
            }
            ...
            $transformedCourses = $courses->getCollection()->map(function($course) {
                ...
                $totalEnrolledStudents = \App\Models\OrderCourse::whereHas('order', function($q) {
                    $q->where('status', 'completed');
                })->where('course_id', $course->id)->count();

                // Get rating information
                $ratings = \App\Models\Rating::where('rateable_type', 'App\\Models\\Course\\Course')
            ...
            $message = $isOwnCourses ? 'Your courses retrieved successfully' : 'Courses where team member is assigned as instructor retrieved successfully';
    ...
    private function getEnrolledStudentsWithProgress($courseId, $perPage = 10, $page = 1)
    {
        ...
        $enrolledStudents = User::whereHas('orders.orderCourses', function($query) use ($courseId) {
            $query->where('course_id', $courseId)
                  ->whereHas('order', function($orderQuery) {
                      $orderQuery->where('status', 'completed');
                  });
        })
        ->with(['orders' => function($query) use ($courseId) {
            $query->whereHas('orderCourses', function($orderCourseQuery) use ($courseId) {
    ...
    public function getCourseAssignmentDetails(Request $request)
    {
        try {
            ...
            if ($request->filled('team_user_slug')) {
                ...
                if (empty($courses)) {
                        return ApiResponseService::successResponse('No courses found for this team', $this->replacePaginationFormat([], 1, 10, 0));
                }
                ...
                $assignmentDetails = $this->getTeamAssignmentDetailsWithPagination($courses, $perPage, $page, $search, $filter);

                return ApiResponseService::successResponse('Team assignment details retrieved successfully', $assignmentDetails);
                ...
                if (!$request->filled('id') && !$request->filled('slug') && !$request->filled('team_user_slug')) {
                    ...
                    if (empty($instructorCourses)) {
                        return ApiResponseService::successResponse('No courses found for this instructor', $this->replacePaginationFormat([], 1, 10, 0));
                    }
                    ...
                    $assignmentDetails = $this->getTeamAssignmentDetailsWithPagination($instructorCourses, $perPage, $page, $search, $filter);

                    return ApiResponseService::successResponse('Instructor assignment details retrieved successfully', $assignmentDetails);
                }
                ...
                $assignmentDetails = $this->getAssignmentDetailsWithPagination($course->id, $perPage, $page, $search, $filter);

                return ApiResponseService::successResponse('Assignment details retrieved successfully', $assignmentDetails);
            }
     ...
     */
    private function getTeamAssignmentDetailsWithPagination($courseIds, $perPage = 10, $page = 1, $search = '', $filter = 'all')
    {
        // Get all assignments for the team courses with their relationships
        $assignmentsQuery = \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::whereIn('course_chapter_id', function($query) use ($courseIds) {
            $query->select('id')
        ...
        ->with([
            ...
            'chapter.lectures' => function($query) {
        ...
        if (!empty($search)) {
            $assignmentsQuery->where(function($query) use ($search) {
                $query->where('title', 'like', '%' . $search . '%')
                      ...
                      ->orWhereHas('chapter.course', function($courseQuery) use ($search) {
                          $courseQuery->where('title', 'like', '%' . $search . '%');
                      })
                      ->orWhereHas('chapter.lectures', function($lectureQuery) use ($search) {
        ...
        $transformedAssignments = $allAssignments->map(function($assignment, $index) {
     ...
     */
    private function getAssignmentDetailsWithPagination($courseId, $perPage = 10, $page = 1, $search = '', $filter = 'all')
    {
        // Get all assignments for the course with their relationships
        $assignmentsQuery = \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::whereHas('chapter', function($query) use ($courseId) {
            $query->where('course_id', $courseId);
        })
        ->with([
            'chapter' => function($query) {
    ...
    public function updateAssignmentStatus(Request $request)
    {
        try {
            ...
            $submission = \App\Models\Course\CourseChapter\Assignment\UserAssignmentSubmission::with(['assignment.chapter.course'])
                ->where('id', $request->submission_id)
                ->whereHas('assignment.chapter.course', function($courseQuery) use ($instructorId) {
                    $courseQuery->where('user_id', $instructorId);
                })
                ->first();

            if (!$submission) {
                return ApiResponseService::validationError('Assignment submission not found or you do not have permission to update it');
            }
    ...
    public function getCourseAssignmentSubmissions(Request $request)
    {
        try {
            ...
            if (!$request->filled('id') && !$request->filled('slug') && !$request->filled('team_user_slug')) {
                ...
                return ApiResponseService::validationError('Please provide either course id or team_user_slug, not both');
            } elseif ($request->filled('slug') && $request->filled('team_user_slug')) {
                return ApiResponseService::validationError('Please provide either course slug or team_user_slug, not both');
            }
            ...
            if ($request->filled('team_user_slug')) {
                ...
                if ($request->filled('assignment_id')) {
                    ...
                    $assignment = \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::where('slug', $request->get('assignment_slug'))->first();
                    if ($assignment) {
                ...
                $submissions = $this->getTeamAssignmentSubmissionsWithPagination($courses, $perPage, $page, $search, $assignmentId, $status);

                return ApiResponseService::successResponse('Team assignment submissions retrieved successfully', $submissions);
                ...
                if (!$request->filled('id') && !$request->filled('slug') && !$request->filled('team_user_slug')) {
                    ...
                    $submissions = $this->getTeamAssignmentSubmissionsWithPagination($instructorCourses, $perPage, $page, $search, $assignmentId, $status);

                    return ApiResponseService::successResponse('Instructor assignment submissions retrieved successfully', $submissions);
                }
                ...
                $submissions = $this->getAssignmentSubmissionsWithPagination($course->id, $perPage, $page, $search, $assignmentId, $status);
     ...
     */
    private function getTeamAssignmentSubmissionsWithPagination($courseIds, $perPage = 10, $page = 1, $search = '', $assignmentId = null, $status = null)
    {
        // Build query for assignment submissions from team courses
        $query = \App\Models\Course\CourseChapter\Assignment\UserAssignmentSubmission::with([
            'user:id,name,email,profile',
            'assignment.chapter.course:id,title,slug',
            'files'
        ])
        ->whereHas('assignment.chapter.course', function($courseQuery) use ($courseIds) {
        ...
        if (!empty($search)) {
            $query->where(function($q) use ($search) {
                ...
                ->orWhereHas('assignment.chapter.course', function($courseQuery) use ($search) {
                    $courseQuery->where('title', 'LIKE', "%{$search}%");
                });
            });
        }
        ...
        $transformedSubmissions = $allSubmissions->map(function($submission) {
            return [
                'id' => $submission->id,
                'user' => [
                    'id' => $submission->user->id,
     ...
     */
    private function getAssignmentSubmissionsWithPagination($courseId, $perPage = 10, $page = 1, $search = '', $assignmentId = null, $status = null)
    {
        ...
        ->whereHas('assignment.chapter.course', function($courseQuery) use ($courseId) {
    ...
    private function getInstructorOverviewStats($instructorId)
    {
        try {
            ...
            $enrolledStudents = OrderCourse::whereHas('course', function($query) use ($instructorId) {
            ...
            $coursesSold = OrderCourse::whereHas('course', function($query) use ($instructorId) {
                $query->where('user_id', $instructorId);
            })
            ->whereHas('order', function($query) {
                $query->where('status', 'completed');
            ...
            // Total earnings - calculate from orders (includes tax, promo, etc.)
            $totalRevenue = Order::whereHas('orderCourses', function($query) use ($instructorId) {
                $query->whereHas('course', function($courseQuery) use ($instructorId) {
                    $courseQuery->where('user_id', $instructorId);
                });
            })
            ->where('status', 'completed')
            ...
            // Get actual commission from Commission table
            $totalCommission = \App\Models\Commission::where('instructor_id', $instructorId)
                ->sum('instructor_commission_amount');

            // Calculate instructor earnings (revenue - commission)
            $instructorEarnings = $totalRevenue - $totalCommission;

            // Average rating/feedback
            $averageRating = Rating::whereHas('rateable', function($query) use ($instructorId) {
                $query->where('user_id', $instructorId);
            })
    ...
    private function getInstructorYearlySalesData($instructorId, $year)
    {
        ...
        for ($month = 1; $month <= 12; $month++) {
            $orders = OrderCourse::whereHas('course', function($query) use ($instructorId) {
                $query->where('user_id', $instructorId);
            })
            ...
            $profit = $this->calculateInstructorProfit($orders);
    ...
    private function getInstructorMonthlySalesData($instructorId, $year, $month)
    {
        ...
        for ($day = 1; $day <= $daysInMonth; $day++) {
            ...
            ->whereHas('order', function($query) use ($year, $month, $day) {
                $query->where('status', 'completed')
                      ->whereYear('created_at', $year)
                      ->whereMonth('created_at', $month)
    ...
    private function getInstructorWeeklySalesData($instructorId, $year, $week)
    {
        ...
        for ($day = 0; $day < 7; $day++) {
            ...
            ->whereHas('order', function($query) use ($currentDay) {
                $query->where('status', 'completed')
    ...
    /**
     * Calculate instructor profit (70% of revenue)
     */
    private function calculateInstructorProfit($orders)
    {
        $revenue = $orders->sum('price');
        return $revenue * 0.7;
    }
    ...
    private function getInstructorMostSellingCourses($instructorId)
    {
        try {
            $courses = Course::where('user_id', $instructorId)
                ->withCount(['orderCourses as sales_count' => function($query) {
                    $query->whereHas('order', function($orderQuery) {
                        $orderQuery->where('status', 'completed');
                    });
                }])
                ->with(['orderCourses' => function($query) {
                    $query->whereHas('order', function($orderQuery) {
                        $orderQuery->where('status', 'completed');
                    });
                }])
            ...
            return $courses->map(function($course) {
                return [
                    ...
                    'status' => 'Sold',
                    'thumbnail' => $course->thumbnail ? asset('storage/' . $course->thumbnail) : asset('img/default-course.jpg'),
                    'slug' => $course->slug
                ];
            });
        ...
        try {
            ...
            if ($request->filled('team_user_slug')) {
                ...
                if (!$request->filled('id') && !$request->filled('slug') && !$request->filled('team_user_slug')) {
                    ...
                    $quizReports = $this->getTeamQuizReportsWithPagination($instructorCourses, $perPage, $page, $search, $categoryId);

                    return ApiResponseService::successResponse('Instructor quiz reports retrieved successfully', $quizReports);
                }
     ...
     */
    private function getTeamQuizReportsWithPagination($courseIds, $perPage = 10, $page = 1, $search = '', $categoryId = null)
    {
        // Get all quizzes for the team courses with their relationships
        $quizzesQuery = \App\Models\Course\CourseChapter\Quiz\CourseChapterQuiz::whereIn('course_chapter_id', function($query) use ($courseIds) {
            $query->select('id')
                  ->from('course_chapters')
                  ->whereIn('course_id', $courseIds);
        })
        ->with([
            ...
            'chapter.course.category' => function($query) {
        ...
        // Filter by category
        if ($categoryId) {
            $quizzesQuery->whereHas('chapter.course', function($courseQuery) use ($categoryId) {
        ...
        if (!empty($search)) {
            $quizzesQuery->where(function($query) use ($search) {
                $query->where('title', 'like', '%' . $search . '%')
                      ...
                      ->orWhereHas('chapter.course.category', function($categoryQuery) use ($search) {
                          $categoryQuery->where('name', 'like', '%' . $search . '%');
                      });
            });
        }
        ...
        $transformedQuizzes = $allQuizzes->map(function($quiz, $index) {
            return [
                'id' => $quiz->id,
    ...
    private function getQuizReportsWithPagination($courseId, $perPage = 10, $page = 1, $search = '', $categoryId = null)
    {
        ...
        $quizzesQuery = \App\Models\Course\CourseChapter\Quiz\CourseChapterQuiz::whereHas('chapter', function($query) use ($courseId) {
            $query->where('course_id', $courseId);
        })
        ->with([
            'chapter' => function($query) {
    ...
    private function getAllResources($courseId)
    {
        ...
        $chapters = \App\Models\Course\CourseChapter\CourseChapter::where('course_id', $courseId)
            ->with([
                ...
                'lectures.resources' => function($query) {
                    $query->where('is_active', true)
                          ->orderBy('order');
                }
            ])
        ...
        foreach ($chapters as $chapter) {
            ...
            foreach ($chapter->resources as $resource) {
                $chapterResourceList[] = [
                    ...
                    'file_name' => $resource->file_extension ? $resource->title . '.' . $resource->file_extension : $resource->title,
                    'file_extension' => $resource->file_extension,
                    'description' => $resource->description,
            ...
            foreach ($chapter->lectures as $lecture) {
                ...
                foreach ($lecture->resources as $resource) {
                    $lectureResourceList[] = [
                        'id' => $resource->id,
                        'title' => $resource->file_extension ? $resource->title . '.' . $resource->file_extension : $resource->title,
                        'type' => $resource->type === 'file' ? 'download' : 'external_link',
                        'file_url' => $resource->type === 'file' ? $resource->file : null,
                        'external_url' => $resource->type === 'url' ? $resource->url : null,
                        'file_extension' => $resource->file_extension,
    ...
    {
        $lecture = \App\Models\Course\CourseChapter\Lecture\CourseChapterLecture::with([
            'resources' => function($query) {
                $query->where('is_active', true)
                      ->orderBy('order');
            },
    ...
    public function getMostSellingCourses(Request $request)
    {
        try {
            ...
            $coursesQuery = Course::where('user_id', $instructorId)
                ...
                ->withCount([
                    'orderCourses as total_sales' => function($query) {
                        $query->whereHas('order', function($orderQuery) {
                            $orderQuery->where('status', 'completed');
                        });
                    }
                ...
                ->withSum([
                    'orderCourses as total_revenue' => function($query) {
                        $query->whereHas('order', function($orderQuery) {
                            $orderQuery->where('status', 'completed');
                        });
                    }
            ...
            if (in_array($filter, ['yearly', 'monthly', 'weekly'])) {
                ...
                $coursesQuery->whereHas('orderCourses', function($query) use ($filter) {
                    $query->whereHas('order', function($orderQuery) use ($filter) {
                        $orderQuery->where('status', 'completed');
                ...
                $coursesQuery->whereHas('orderCourses', function($query) {
                    $query->whereHas('order', function($orderQuery) {
                        $orderQuery->where('status', 'completed');
                    });
                });
            ...
            switch ($filter) {
                ...
                default:
                    ...
                    $coursesQuery->orderBy('total_sales', 'desc');
                    break;
            }
    ...
    {
        $query = \App\Models\OrderCourse::where('course_id', $courseId)
            ->whereHas('order', function($orderQuery) {
                $orderQuery->where('status', 'completed');
            });
    ...
    private function calculateStudentProgress($userId, $courseId)
    {
        ...
        $course = Course::with([
            ...
            'chapters.resources' => function($query) {
                $query->where('is_active', 1);
            }
        ])->find($courseId);
        ...
        foreach ($course->chapters as $chapter) {
            ...
            foreach ($chapter->lectures as $lecture) {
                $totalItems++;
                $isCompleted = \App\Models\UserCurriculumTracking::where('user_id', $userId)
                    ->where('course_chapter_id', $chapter->id)
            ...
            foreach ($chapter->assignments as $assignment) {
                ...
                $isCompleted = \App\Models\UserCurriculumTracking::where('user_id', $userId)
                    ...
                    ->where('model_type', \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::class)
                    ->where('status', 'completed')
                    ->exists();
                if ($isCompleted) $completedItems++;
            }
    ...
    {
        try {
            // Get total earnings from this course
            $totalEarnings = OrderCourse::whereHas('order', function ($q) {
                $q->where('status', 'completed');
            })->where('course_id', $courseId)->sum('price');

            // Get total enrolled users
            $totalEnrolledUsers = OrderCourse::whereHas('order', function ($q) {
                $q->where('status', 'completed');
            })->where('course_id', $courseId)->count();

            // Get total reviews count
            $totalReviews = Rating::where('rateable_type', 'App\\Models\\Course\\Course')
                ->where('rateable_id', $courseId)->count();

            // Get total sales count (same as enrolled users for courses)
            $totalSales = $totalEnrolledUsers;
    ...
    private function getMonthlySalesData($courseId)
    {
        ...
        for ($i = 11; $i >= 0; $i--) {
            ...
            $monthlySales = OrderCourse::whereHas('order', function ($q) use ($date) {
                $q->where('status', 'completed')
            ...
            $monthlyRevenue = OrderCourse::whereHas('order', function ($q) use ($date) {
                $q->where('status', 'completed')
                  ->whereYear('created_at', $date->year)
                  ->whereMonth('created_at', $date->month);
            })->where('course_id', $courseId)->sum('price');
    ...
    private function getCourseYearlySalesData($courseId, $year)
    {
        ...
        for ($month = 1; $month <= 12; $month++) {
            $orders = OrderCourse::whereHas('order', function ($q) use ($year, $month) {
                $q->where('status', 'completed')
                  ->whereYear('created_at', $year)
                  ->whereMonth('created_at', $month);
            ...
            $sales = $orders->count();
            $revenue = $orders->sum('price');
            $profit = $this->calculateCourseProfit($orders);
    ...
    private function getCourseMonthlySalesData($courseId, $year, $month)
    {
        ...
        for ($day = 1; $day <= $daysInMonth; $day++) {
            $orders = OrderCourse::whereHas('order', function ($q) use ($year, $month, $day) {
                $q->where('status', 'completed')
                  ->whereYear('created_at', $year)
                  ->whereMonth('created_at', $month)
                  ->whereDay('created_at', $day);
    ...
    private function getCourseWeeklySalesData($courseId, $year, $week)
    {
        ...
        while ($currentDate->lte($endOfWeek)) {
            $orders = OrderCourse::whereHas('order', function ($q) use ($currentDate) {
                $q->where('status', 'completed')
    ...
    /**
     * Calculate profit for course orders
     */
    private function calculateCourseProfit($orders)
    {
        // For now, using a simple calculation: 70% of revenue as profit
        // You can modify this based on your actual profit calculation logic
        $revenue = $orders->sum('price');
        return $revenue * 0.7;
    }
    ...
    private function getCourseContentStatistics($courseId)
    {
        try {
            ...
            $totalQuizzes = $course->chapters->sum(function ($chapter) {
                return $chapter->quizzes->count();
            });
            $totalAssignments = $course->chapters->sum(function ($chapter) {
                return $chapter->assignments->count();
            });

            // Calculate total duration
            $totalDuration = $course->chapters->sum(function ($chapter) {
                return $chapter->lectures->sum('duration');
            });
            ...
            return [
                ...
                'content_breakdown' => [
                    'lectures_percentage' => $totalLectures > 0 ? round(($totalLectures / ($totalLectures + $totalQuizzes + $totalAssignments)) * 100, 1) : 0,
                    'quizzes_percentage' => $totalQuizzes > 0 ? round(($totalQuizzes / ($totalLectures + $totalQuizzes + $totalAssignments)) * 100, 1) : 0,
                    'assignments_percentage' => $totalAssignments > 0 ? round(($totalAssignments / ($totalLectures + $totalQuizzes + $totalAssignments)) * 100, 1) : 0
                ]
            ];
            ...
            return [
                ...
                'content_breakdown' => ['lectures_percentage' => 0, 'quizzes_percentage' => 0, 'assignments_percentage' => 0],
    ...
    private function getCourseStatistics($userId, $teamUser = null)
    {
        ...
        if ($teamUser) {
            $baseQuery->whereHas('instructors', function($q) use ($teamUser) {
    ...
    {
        try {
            $courses = UserCourseTrack::where('user_id', Auth::user()->id)
    ...
    public function getMyLearning(Request $request)
    {
        try {
            ...
            $enrolledCoursesQuery = Order::where('user_id', $userId)
                ->where('status', 'completed')
                ->with([
                    'orderCourses.course' => function($query) {
                        $query->with([
                            'category',
                            'user',
                            'taxes',
            ...
            $refundedCoursesByOrder = DB::table('refund_requests')
                ...
                ->map(function($refunds) {
                    return $refunds->mapWithKeys(function($refund) {
                        $orderId = (int) $refund->order_id;
                        $refundDate = $refund->processed_at ?? $refund->created_at;
                        return [$orderId => \Carbon\Carbon::parse($refundDate)];
                    });
                });
            ...
            foreach ($orders as $order) {
                ...
                foreach ($order->orderCourses as $orderCourse) {
                    ...
                    if ($orderCourse->course && 
                        ...
                        $existingIndex = $enrolledCoursesWithPurchaseDate->search(function($item) use ($courseId) {
            ...
            if ($request->filled('search')) {
                $search = $request->search;
                $enrolledCourses = $enrolledCourses->filter(function($course) use ($search) {
                    return $course && (
                        stripos($course->title, $search) !== false ||
                        stripos($course->short_description, $search) !== false ||
                        stripos($course->level, $search) !== false ||
                        ($course->category && stripos($course->category->name, $search) !== false) ||
                        ($course->user && stripos($course->user->name, $search) !== false)
                    );
                })->values();
            }
            ...
            if ($request->filled('category_id')) {
                $enrolledCourses = $enrolledCourses->filter(function($course) use ($request) {
                    return $course && $course->category && $course->category->id == $request->category_id;
                })->values();
            }

            if ($request->filled('level')) {
                $levels = explode(',', $request->level);
                $enrolledCourses = $enrolledCourses->filter(function($course) use ($levels) {
                    return $course && in_array($course->level, $levels);
                })->values();
            }
            ...
            if ($request->filled('course_type')) {
                ...
                if ($courseTypeFilter === 'free') {
                    ...
                    $enrolledCourses = $enrolledCourses->filter(function($course) {
                        return $course && $course->course_type === 'free';
                    })->values();
            ...
            $enrolledCourses = $enrolledCourses->sortBy(function($course) use ($sortBy, $purchaseDatesMap) {
                // Skip null courses
                if (!$course) {
                    return null;
                }
                ...
                switch ($sortBy) {
                    case 'purchase_date':
                        ...
                        return $purchaseDatesMap[$course->id] ?? $course->created_at;
                    case 'id':
                        return $course->id;
                    case 'title':
                        return $course->title;
            ...
            $coursesData = $enrolledCourses->filter(function($course) {
                return $course !== null;
            })->values();
            ...
            $transformedCourses = $coursesData->map(function ($course) use ($userId, $refundEnabled, $refundPeriodDays, $purchaseDatesMap) {
                // Skip null courses
                if (!$course) {
                    return null;
                }
                ...
                $totalChapters = $courseWithChapters->chapters->count();


                // Calculate total curriculum items (lectures + quizzes + assignments + resources)
                $totalCurriculumItems = $courseWithChapters->chapters->sum(function ($chapter) {
                    return $chapter->lectures->count() + 
                           $chapter->quizzes->count() + 
                           $chapter->assignments->count() + 
                ...
                if ($completedChapters > 0 && $lastCompletedChapterId) {
                    ...
                    $lastCompletedChapter = $courseWithChapters->chapters->firstWhere('id', $lastCompletedChapterId);
                ...
                $orderDate = $purchaseDatesMap[$course->id] ?? Order::where('user_id', $userId)
                    ->where('status', 'completed')
                    ->whereHas('orderCourses', function($q) use ($course) {
            ...
            if ($request->filled('progress_status') && $request->progress_status !== 'all') {
                ...
                $transformedCourses = $transformedCourses->filter(function($course) use ($progressStatus) {
    ...
    public function userTrackCourse(Request $request)
    {
        ...
        try {
            ...
            if ($hasApprovedRefund) {
                ApiResponseService::validationError('Refund is approved for this course. Progress tracking is disabled.');
            }
    ...
    private function getCourseQuizReports($courseId, $quizId = null)
    {
        try {
            ...
            if ($quizId) {
                ...
                $chaptersQuery->whereHas('quizzes', function($q) use ($quizId) {
                    $q->where('id', $quizId);
                });
            }
            
            $chapters = $chaptersQuery->with(['quizzes' => function($query) {
                $query->with([
                    'questions',
                    'attempts' => function($attemptQuery) {
                        $attemptQuery->with(['user', 'answers.option'])
                            ->orderBy('created_at', 'desc');
                    }
                ]);
            }])->get();
            ...
            if ($quizId) {
                $quizzes = $quizzes->filter(function($quiz) use ($quizId) {
            ...
            $quizDetails = $quizzes->map(function($quiz) {
                ...
                return [
                    ...
                    'pass_rate' => $attempts->count() > 0 ? 
                        round(($scores->filter(function($score) { return $score >= 70; })->count() / $attempts->count()) * 100, 1) : 0,
            ...
            usort($studentAttempts, function($a, $b) {
                return strtotime($b['last_attempt_date']) - strtotime($a['last_attempt_date']);
            });
            ...
            \Illuminate\Support\Facades\Log::error('Error getting course quiz reports: ' . $e->getMessage());
            return [
                'total_quizzes' => 0,
                'total_attempts' => 0,
                'average_score' => 0,
    ...
    public function getQuizAttemptDetails(Request $request)
    {
        try {
            ...
            $attempt = \App\Models\Course\CourseChapter\Quiz\UserQuizAttempt::with([
                'user',
                'quiz.chapter.course',
            ...
            if ($attempt->answers->count() > 0) {
                foreach ($attempt->answers as $answer) {
                    if ($answer->option && $answer->option->question) {
                        ...
                        $options = \App\Models\Course\CourseChapter\Quiz\QuizOption::where('quiz_question_id', $question->id)->get();
                        ...
                        $questions[] = [
                            ...
                            'options' => $options->map(function($option) use ($answer) {
                                ...
                                return [
                                    ...
                                    'status' => $isSelected ? ($isCorrect ? 'correct' : 'incorrect') : ($isCorrect ? 'correct_answer' : 'not_selected')
                                ];
                            }),
                            'student_answer' => [
            ...
            \Illuminate\Support\Facades\Log::error('Error getting quiz attempt details: ' . $e->getMessage());
    ...
    private function getQuizAttemptDetailsForCourse($attemptId, $courseId)
    {
        try {
            ...
            \Illuminate\Support\Facades\Log::error('Error getting quiz attempt details for course: ' . $e->getMessage());
            return [
    ...
    public function getQuizReportDetails(Request $request)
    {
        try {
            ...
            $quizQuery = \App\Models\Course\CourseChapter\Quiz\CourseChapterQuiz::with([
                'chapter.course',
                'questions',
            ...
            if ($request->filled('team_user_slug')) {
                ...
                if (!$isInSameTeam) {
                    return ApiResponseService::validationError('You are not authorized to access this quiz data. You are not in the same team.');
                }
            }
            ...
            $passedAttempts = $attempts->filter(function($attempt) use ($passingPoints) {
                return $attempt->earned_points >= $passingPoints;
            });
            $failedAttempts = $attempts->filter(function($attempt) use ($passingPoints) {
                return $attempt->earned_points < $passingPoints;
            });
            ...
            foreach ($studentAttempts as $userId => $userAttempts) {
                ...
                $bestScore = $userAttempts->max('earned_points');
                ...
                $studentPerformance[] = [
                    ...
                    'player_image' => $user->profile ? (filter_var($user->profile, FILTER_VALIDATE_URL) ? $user->profile : \Illuminate\Support\Facades\Storage::url($user->profile)) : null,
            ...
            if ($searchTerm) {
                $studentPerformance = array_filter($studentPerformance, function($student) use ($searchTerm) {
                    $searchLower = strtolower($searchTerm);
                    return strpos(strtolower($student['player_name']), $searchLower) !== false ||
                           strpos(strtolower($student['player_email']), $searchLower) !== false;
                });
            }
            ...
            if ($statusFilter !== 'all') {
                $studentPerformance = array_filter($studentPerformance, function($student) use ($statusFilter) {
            ...
            \Illuminate\Support\Facades\Log::error('Error getting quiz report details: ' . $e->getMessage());
    ...
    public function getQuizResultDetails(Request $request)
    {
        try {
            ...
            // Get the attempt with all related data
            $attemptQuery = \App\Models\Course\CourseChapter\Quiz\UserQuizAttempt::with([
                'user',
                'quiz.chapter.course',
                'answers.option.question'
            ]);
            ...
            foreach ($questions as $index => $question) {
                ...
                foreach ($question->options as $option) {
                    ...
                    $optionsData[] = [
                        ...
                        'status' => $isSelected ? ($isCorrectAnswer ? 'correct' : 'incorrect') : ($isCorrectAnswer ? 'correct_answer' : 'normal')
                    ];
                }
            ...
            \Illuminate\Support\Facades\Log::error('Error getting quiz result details: ' . $e->getMessage());
    ...
    private function getCourseDiscussions($courseId)
    {
        try {
            ...
            $questions = \App\Models\HelpdeskQuestion::with(['user', 'replies.user', 'group'])
            ...
            $formattedDiscussions = $questions->map(function($question) {
                // Get user avatar
                $userAvatar = $question->user->profile ?? 
                    'https://via.placeholder.com/40x40/' . substr(md5($question->user->name), 0, 6) . '/000000?text=' . substr($question->user->name, 0, 2);
                ...
                $formattedReplies = $question->replies->whereNull('parent_id')->map(function($reply) {
                    $replyUserAvatar = $reply->user->profile ?? 
                        'https://via.placeholder.com/40x40/' . substr(md5($reply->user->name), 0, 6) . '/000000?text=' . substr($reply->user->name, 0, 2);
            ...
            $totalReplies = $questions->sum(function($question) {
                return $question->replies->count();
            });
            $activeUsers = $questions->pluck('user.id')->merge(
                $questions->flatMap->replies->pluck('user.id')
            )->unique()->count();
            ...
            \Illuminate\Support\Facades\Log::error('Error getting helpdesk discussions: ' . $e->getMessage());
            return [
        ...
        try {
            ...
            $reviews = $ratings->map(function($rating) {
                return [
                    ...
                    'user' => [
                        ...
                        'avatar' => $rating->user->profile ?? 
                            'https://via.placeholder.com/40x40/' . substr(md5($rating->user->name ?? 'user'), 0, 6) . '/000000?text=' . substr($rating->user->name ?? 'U', 0, 1),
            ...
            return [
                ...
                'summary' => [
                    ...
                    'most_common_rating' => $ratingBreakdown['5_stars'] > 0 ? 5 : 
                                          ($ratingBreakdown['4_stars'] > 0 ? 4 : 
                                          ($ratingBreakdown['3_stars'] > 0 ? 3 : 
                                          ($ratingBreakdown['2_stars'] > 0 ? 2 : 1)))
                ]
            ];

        } catch (\Exception $e) {
            \Illuminate\Support\Facades\Log::error('Error getting course ratings: ' . $e->getMessage());
            return [
    ...
    {
        try {
            $assignments = \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::whereHas('chapter', function($query) use ($courseId) {
                    $query->where('course_id', $courseId);
                })
            ...
            \Illuminate\Support\Facades\Log::error('Error getting course assignments: ' . $e->getMessage());
            return [];
        }
    }
    ...
    {
        try {
            $query = \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::whereHas('chapter', function($query) use ($courseId) {
                    $query->where('course_id', $courseId);
                })
                ->where('is_active', true)
            ...
            \Illuminate\Support\Facades\Log::info('Assignments found: ' . $assignments->count());
            foreach ($assignments as $assignment) {
                \Illuminate\Support\Facades\Log::info('Assignment ID: ' . $assignment->id . ', Submissions count: ' . $assignment->submissions->count());
                
                // Check if submissions relationship is loaded
                if ($assignment->relationLoaded('submissions')) {
                    \Illuminate\Support\Facades\Log::info('Submissions relationship is loaded');
                    \Illuminate\Support\Facades\Log::info('Raw submissions: ' . $assignment->submissions->toJson());
                } else {
                    \Illuminate\Support\Facades\Log::info('Submissions relationship is NOT loaded');
                }
                
                // Try to get submissions manually
                $manualSubmissions = \App\Models\Course\CourseChapter\Assignment\UserAssignmentSubmission::where('course_chapter_assignment_id', $assignment->id)->get();
                \Illuminate\Support\Facades\Log::info('Manual query submissions count: ' . $manualSubmissions->count());
                \Illuminate\Support\Facades\Log::info('Manual query submissions: ' . $manualSubmissions->toJson());
                
                // Also check the database directly
                $dbSubmissions = \Illuminate\Support\Facades\DB::table('user_assignment_submissions')
                    ->where('course_chapter_assignment_id', $assignment->id)
                    ->get();
                \Illuminate\Support\Facades\Log::info('Direct DB query submissions count: ' . $dbSubmissions->count());
                \Illuminate\Support\Facades\Log::info('Direct DB query submissions: ' . $dbSubmissions->toJson());
                
                // Check if there's a mismatch in the relationship
                \Illuminate\Support\Facades\Log::info('Assignment table: course_chapter_assignments, ID: ' . $assignment->id);
                \Illuminate\Support\Facades\Log::info('Submissions table: user_assignment_submissions, looking for course_chapter_assignment_id: ' . $assignment->id);
            }

            return $assignments->map(function($assignment) {
                // Get all submissions for this assignment
                $allSubmissions = $assignment->submissions->map(function($submission) {
                    return [
                        'id' => $submission->id,
            ...
            \Illuminate\Support\Facades\Log::error('Error getting assignment details: ' . $e->getMessage());
    ...
    public function getSearchSuggestions(Request $request)
    {
        try {
            ...
            if (!empty($query)) {
                \App\Models\SearchHistory::recordSearch($query, $userId, $ipAddress);
            }

            // Get recent searches
            $recentSearches = \App\Models\SearchHistory::getRecentSearches($userId, $ipAddress, 5)
                ->map(function ($search) {
                    return [
                        'type' => 'recent',
                        'text' => $search->query,
            ...
            $courseSuggestions = $courses->map(function ($course) {
            ...
            if (empty($query)) {
                ...
                $courseSuggestions = $popularCourses->map(function ($course) {
                    // Get primary author (course creator or first instructor)
                    $author = $course->user;
                    if ($course->instructors->isNotEmpty()) {
                        $author = $course->instructors->first();
    ...
    public function getCourseReviews(Request $request)
    {
        try {
            ...
            if ($isSpecificCourse) {
                $allRatings = Rating::where('rateable_type', 'App\\Models\\Course\\Course')
                    ->where('rateable_id', $course->id)
                    ->get();
            ...
            if ($user && $isSpecificCourse) {
                $userRating = Rating::where('rateable_type', 'App\\Models\\Course\\Course')
                    ->where('rateable_id', $course->id)
                    ->where('user_id', $user->id)
                    ->first();
    ...
    public function getInstructorReviews(Request $request)
    {
        try {
            ...
            $instructor = null;
            if ($request->filled('instructor_id')) {
                $instructor = \App\Models\Instructor::where('id', $request->instructor_id)
                    ->first();
            } elseif ($request->filled('slug')) {
                // Find user by slug first, then get their instructor record
                $instructorUser = User::where('slug', $request->slug)->first();
                if ($instructorUser) {
                    $instructor = \App\Models\Instructor::where('user_id', $instructorUser->id)->first();
                }
            } else {
            ...
            $query = Rating::with(['user'])
                ->where('rateable_type', 'App\\Models\\Instructor')
            ...
            $allRatings = Rating::where('rateable_type', 'App\\Models\\Instructor')
                ->where('rateable_id', $instructor->id)
                ->get();
            ...
            $myReview = null;
            if ($user) {
                $userRating = Rating::where('rateable_type', 'App\\Models\\Instructor')
                    ->where('rateable_id', $instructor->id)
                    ->where('user_id', $user->id)
                    ->first();
    ...
    private function getTeamMemberInstructorReviews(Request $request, $user)
    {
        try {
            ...
            $teamUser = \App\Models\User::where('slug', $teamUserSlug)->first();
    ...
    private function getAuthenticatedUserInstructorReviews(Request $request, $user)
    {
        try {
            ...
            ApiResponseService::logErrorResponse($e, "API Course Controller -> getAuthenticatedUserInstructorReviews Method");
    ...
    public function getDiscussion(Request $request)
    {
        try {
            ...
            $query = \App\Models\Course\CourseDiscussion::with(['user', 'replies.user'])
                ->where('course_id', $course->id)
            ...
            $totalDiscussions = \App\Models\Course\CourseDiscussion::where('course_id', $course->id)
                ->whereNull('parent_id')
                ->count();
            ...
            $formattedDiscussions = $discussions->map(function($discussion) {
                ...
                return [
                    ...
                    'author' => [
                        ...
                        'avatar' => $discussion->user->profile ?? 
                            'https://via.placeholder.com/40x40/' . substr(md5($discussion->user->name ?? 'user'), 0, 6) . '/000000?text=' . substr($discussion->user->name ?? 'U', 0, 1),
                        ...
                        return [
                            ...
                            'author' => [
                                ...
                                'avatar' => $reply->user->profile ?? 
                                    'https://via.placeholder.com/40x40/' . substr(md5($reply->user->name ?? 'user'), 0, 6) . '/000000?text=' . substr($reply->user->name ?? 'U', 0, 1),
    ...
    public function replyDiscussion(Request $request)
    {
        try {
            ...
            // Get the discussion
            $discussion = \App\Models\Course\CourseDiscussion::with('course')->find($request->discussion_id);
            if (!$discussion) {
            ...
            // Create the reply
            $reply = \App\Models\Course\CourseDiscussion::create([
            ...
            $formattedReply = [
                ...
                'author' => [
                    ...
                    'name' => $reply->user->name,
                    'avatar' => $reply->user->profile ?? 
                        'https://via.placeholder.com/40x40/' . substr(md5($reply->user->name), 0, 6) . '/000000?text=' . substr($reply->user->name, 0, 1),
                    'email' => $reply->user->email,
                    'is_instructor' => true
                ],
                'created_at' => $reply->created_at->format('M d, Y'),
        ...
        try {
            ...
            if ($personalDetails) {
                ...
                $missingFields = array_merge($missingFields, [
                    'About Me', 'Qualification', 'Years of Experience', 
                    'Skills', 'Team Name', 'Team Logo'
                ]);
            }
    ...
    public function generateCourseCertificate(Request $request)
    {
        try {
            ...
            if (!$user) {
                return ApiResponseService::errorResponse('User not authenticated. Please provide a valid authorization token.', null, 401);
            }
    ...
    public function generateExamCertificate(Request $request)
    {
        try {
            ...
            if (!$isExamCompleted) {
                return ApiResponseService::errorResponse('Exam must be completed before generating certificate', null, 400);
            }
    ...
    public function getCertificateTemplates(Request $request)
    {
        try {
            ...
            $formattedTemplates = $templates->map(function ($certificate) {
                return [
                    'id' => $certificate->id,
            ...
            return ApiResponseService::successResponse('Certificate templates fetched successfully', $formattedTemplates);
    ...
    private function checkCourseCompletion($userId, $courseId)
    {
        ...
        $completedLectures = $completedTracking->where('model_type', \App\Models\Course\CourseChapter\Lecture\CourseChapterLecture::class)->count();
        $completedQuizzes = $completedTracking->where('model_type', \App\Models\Course\CourseChapter\Quiz\CourseChapterQuiz::class)->count();
        $completedResources = $completedTracking->where('model_type', \App\Models\Course\CourseChapter\Resource\CourseChapterResource::class)->count();
        ...
        if (!empty($assignmentIds)) {
            $submittedAssignments = \App\Models\Course\CourseChapter\Assignment\UserAssignmentSubmission::where('user_id', $userId)
                ->whereIn('course_chapter_assignment_id', $assignmentIds)
                ->whereIn('status', ['submitted', 'accepted'])
                ->count();
        }

        // Check if all assignments are submitted
        $allAssignmentsSubmitted = ($totalAssignments == 0) || ($submittedAssignments >= $totalAssignments);
