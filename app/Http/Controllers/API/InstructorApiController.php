...
use App\Notifications\InstructorSubmissionNotification;
...
class InstructorApiController extends Controller {
    ...
    public function updateDetails(Request $request)
    {
        ...
        try {
            ...
            if ($instructor && $instructor->status === 'suspended') {
                return ApiResponseService::errorResponse('Your instructor account has been suspended. You cannot update your details.');
            }
                        ...
                        if ($customFormField && $customFormField->is_required == 1) {
                            ...
                            switch ($customFormField->type) {
                                ...
                                default:
                                    ...
                                    if (isset($otherDetail['value']) && !empty(trim($otherDetail['value']))) {
                                        $hasValue = true;
                                    }
                                    break;
                            }
            ...
            if($request->hasFile('team_logo')) {
                $existingFile = !empty($instructorPersonalDetail) ? $instructorPersonalDetail->getRawOriginal('team_logo') : null;
                $personalDetailsData['team_logo'] = FileService::compressAndReplace($request->team_logo, $this->instructorPersonalDetailFolder, $existingFile);
            }
            if($request->hasFile('id_proof')) {
                $existingFile = !empty($instructorPersonalDetail) ? $instructorPersonalDetail->getRawOriginal('id_proof') : null;
                $personalDetailsData['id_proof'] = FileService::compressAndReplace($request->id_proof, $this->instructorPersonalDetailFolder, $existingFile);
            }
            if($request->hasFile('preview_video')) {
                $existingFile = !empty($instructorPersonalDetail) ? $instructorPersonalDetail->getRawOriginal('preview_video') : null;
                $personalDetailsData['preview_video'] = FileService::compressAndReplace($request->preview_video, $this->instructorPersonalDetailFolder, $existingFile);
            }
            ...
            if ($request->has('other_details')) {
                ...
                foreach ($request->other_details as $otherDetail) {
                    ...
                    if (!$customFormField) {
                        ApiResponseService::validationError('Custom form field ID :- ' . $otherDetail['id'] . ' not found');
                    }
    ...
    public function addTeamMember(Request $request)
    {
        ...
        try {
            ...
            // Check if user is already in this team with approved status
            $existsInSameTeam = TeamMember::where([
                'user_id' => $user->id,
                'instructor_id' => $instructor->id,
                'status' => 'approved'
            ])->exists();
            ...
            // Check if there's already a pending invitation - if exists, update it instead of creating new
            $pendingInvitation = TeamMember::where([
                'user_id' => $user->id,
                'instructor_id' => $instructor->id,
                'status' => 'pending'
            ])->first();
            ...
            if ($pendingInvitation) {
                // Update existing pending invitation (regenerate token, update timestamp)
                ...
                try {
                    ...
                    \Illuminate\Support\Facades\Mail::send('emails.team-invitation', [
                        'user' => $user,
                        ...
                        $message->to($user->email, $user->name)
                                ->subject('Team Invitation - ' . $appName);
                    });
                } catch (\Exception $e) {
                    \Illuminate\Support\Facades\Log::error('Failed to resend team invitation email: ' . $e->getMessage());
                }

                // Resend notification (send immediately, not queued)
                try {
                    $notification = new TeamInvitationNotification($loggedInUser, $pendingInvitation);
                    $user->notify($notification);
                } catch (\Exception $e) {
                    \Illuminate\Support\Facades\Log::error('Failed to resend team invitation notification: ' . $e->getMessage(), [
                        'trace' => $e->getTraceAsString(),
                        'user_id' => $user->id ?? null
                    ]);
                }

                return ApiResponseService::successResponse('Invitation resent successfully.');
            }
            ...
            try {
                ...
                if (!$existingNotification) {
                    ...
                    try {
                        ...
                        \Illuminate\Support\Facades\Log::error('Failed to insert notification directly: ' . $insertError->getMessage(), [
                            'user_id' => $user->id,
                ...
                \Illuminate\Support\Facades\Log::error('Failed to send team invitation notification: ' . $e->getMessage(), [
                    'trace' => $e->getTraceAsString(),
                    'file' => $e->getFile(),
                    'line' => $e->getLine(),
                    'user_id' => $user->id ?? null,
            ...
            return ApiResponseService::successResponse('Invitation sent successfully. User will be added to team after accepting the invitation.');
        } catch (Exception $e) {
     ...
     */
    public function acceptTeamInvitation(Request $request, $token = null)
    {
        try {
            ...
            if ($action === 'accept') {
                ...
                if (!$user->hasRole(config('constants.SYSTEM_ROLES.INSTRUCTOR'))) {
                    return ApiResponseService::validationError('You must have an Instructor role to accept this invitation. Please contact the administrator to get the Instructor role first.');
                }
                ...
                if ($instructorUser) {
                    try {
                        ...
                        \Illuminate\Support\Facades\Log::error('Failed to send team invitation acceptance notification: ' . $e->getMessage());
                    }
                }

                return ApiResponseService::successResponse('Team invitation accepted successfully. You have been added to the team.');
            } else {
                ...
                if ($instructorUser) {
                    try {
                        ...
                        \Illuminate\Support\Facades\Log::error('Failed to send team invitation rejection notification: ' . $e->getMessage());
                    }
                }
    ...
    public function getTeamMembers(Request $request)
    {
        try {
            ...
            $query = TeamMember::where('instructor_id', $instructorId)
                ->with([
                    'user' => function($userQuery) {
                        $userQuery->select('id', 'name', 'email', 'slug', 'profile', 'is_active', 'created_at')
                            ->with(['instructor_details' => function($instructorQuery) {
            ...
            if ($request->filled('course_id')) {
                $query->whereHas('user.courses', function($courseQuery) use ($request) {
            ...
            $invitorsQuery = TeamMember::where('user_id', $loggedInUserData->id)
                ...
                ->with([
                    'instructor' => function($instructorQuery) {
                        $instructorQuery->with(['user' => function($userQuery) {
                            $userQuery->select('id', 'name', 'email', 'slug', 'profile', 'is_active', 'created_at')
                                ->with(['instructor_details' => function($instructorDetailsQuery) {
            ...
            $invitors = $invitorsQuery->map(function ($teamMember) {
                $courses = [];
                ...
                if ($teamMember->instructor && $teamMember->instructor->user) {
                    ...
                    $userCourses = \App\Models\Course\Course::where('user_id', $instructorUserId)
                        ->select('id', 'title', 'slug', 'thumbnail', 'price', 'discount_price', 'status', 'is_active', 'user_id', 'created_at')
                        ->get();
                ...
                return [
                    ...
                    'user_id' => $teamMember->instructor && $teamMember->instructor->user ? $teamMember->instructor->user->id : null,
                    ...
                    'user' => $teamMember->instructor && $teamMember->instructor->user ? [
                        ...
                        'instructor_status' => $teamMember->instructor->user->instructor_details ? $teamMember->instructor->user->instructor_details->status : null,
                    ] : null,
                    'courses' => $courses,
            ...
            $teamMembers->getCollection()->transform(function ($teamMember) use ($loggedInUserData) {
                $courses = [];
                ...
                if ($teamMember->user) {
                    ...
                    if (!empty($assignedCourseIds)) {
                        $assignedCourses = \App\Models\Course\Course::whereIn('id', $assignedCourseIds)
                            ->select('id', 'title', 'slug', 'thumbnail', 'price', 'discount_price', 'status', 'is_active', 'user_id', 'created_at')
                            ->get();
                ...
                if ($teamMember->user && $teamMember->user->instructor_details) {
                    $isInvitor = \App\Models\TeamMember::where('instructor_id', $teamMember->user->instructor_details->id)
                        ->where('user_id', $loggedInUserData->id)
                        ->where('status', 'approved')
                        ->exists();
                ...
                return [
                    ...
                    'user' => $teamMember->user ? [
                        ...
                        'instructor_status' => $teamMember->user->instructor_details ? $teamMember->user->instructor_details->status : null,
                    ] : null,
                    'courses' => $courses,
                    'total_courses' => count($courses)
                ];
    ...
    public function getInvitors(Request $request)
    {
        try {
            ...
            if ($request->filled('search')) {
                $search = $request->search;
                $query->whereHas('instructor.user', function($userQuery) use ($search) {
                    $userQuery->where('name', 'LIKE', "%{$search}%")
                            ->orWhere('email', 'LIKE', "%{$search}%");
                });
            }
            ...
            // Transform data
            $invitors->getCollection()->transform(function ($teamMember) {
                return [
                    'id' => $teamMember->id,
                    'instructor_id' => $teamMember->instructor_id,
                    'user_id' => $teamMember->user_id,
...
if ($request->filled('category_id') || $request->filled('category_slug')) {
    ...
    $categorySlugs = $request->filled('category_slug')
        ? explode(',', $request->category_slug)
        : [];

    $query->where(function ($q) use ($categoryIds, $categorySlugs) {

        // Courses owned by instructor's user_id
        $q->whereHas('user', function ($uq) use ($categoryIds, $categorySlugs) {
            $uq->whereHas('courses', function ($cq) use ($categoryIds, $categorySlugs) {
                if ($categoryIds) {
                    $cq->whereIn('category_id', $categoryIds);
                }
                if ($categorySlugs) {
                    $cq->whereHas('category', function ($cat) use ($categorySlugs) {
        ...
        $q->orWhereHas('courses', function ($cq) use ($categoryIds, $categorySlugs) {
            if ($categoryIds) {
                $cq->whereIn('category_id', $categoryIds);
            }
            if ($categorySlugs) {
        ...
        if ($request->filled('rating')) {
            ...
            $query->havingRaw('ratings_avg_rating >= ?', [$rating])
                  ->havingRaw('ratings_avg_rating < ?', [$rating + 1]);
        }
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->whereHas('user', fn($u) => $u->where('name', 'LIKE', "%$search%")
                    ->orWhere('email', 'LIKE', "%$search%"))
                  ->orWhereHas('personal_details', fn($p) => $p->where('qualification', 'LIKE', "%$search%")
        ...
        $purchasedCourseIds = [];
        if ($authUser) {
            $purchasedCourseIds = \App\Models\OrderCourse::whereHas('order', function($q) use ($authUser) {
        ...
        // ✅ Transform Response
        $data = $instructors->map(function ($instructor) use ($request, $authUser, $purchasedCourseIds) {
            // Count only active courses (is_active = 1 and not soft deleted)
            ...
            $studentEnrolledCount = \App\Models\OrderCourse::whereHas('course', fn($q) => $q->where('user_id', $instructor->user_id))
            ...
            $reviewCount = \App\Models\Rating::where('rateable_type', 'App\Models\Instructor')
            ...
            if ($authUser && !empty($purchasedCourseIds)) {
                ...
                if (!$hasPurchased) {
                    $hasPurchased = \App\Models\Course\Course::whereHas('instructors', function($q) use ($instructor) {
            ...
            return [
                ...
                'social_medias' => $instructor->social_medias->map(function($social) {
                    return [
                        'title' => $social->title ?? '',
                        'url' => $social->url ?? '',
                    ];
    ...
    public function getInstructorDetails(Request $request)
    {
        try {
            ...
            $studentEnrolledCount = \App\Models\OrderCourse::whereHas('course', function($q) use ($instructor) {
                    $q->where('user_id', $instructor->user_id);
                })
            ...
            // Get published courses count (consistent with get-instructors API)
            $publishedCoursesCount = \App\Models\Course\Course::where('user_id', $instructor->user_id)
                ->where('is_active', 1)
                ->where('approval_status', 'approved')
                ->count();
            ...
            $authUserId = null;
            if ($authUser) {
                $myReviewData = \App\Models\Rating::where('rateable_type', 'App\Models\Instructor')
                    ->where('rateable_id', $instructor->id)
            ...
            $ratingsQuery = $instructor->ratings()
                ->with('user')
                ->latest();
            ...
            $ratingsList = $ratingsQuery->limit(10)
                ->get()
                ->map(function($rating) {
                    return [
                        'id' => $rating->id,
    ...
    public function storePromoCodeByInstructor(Request $request)
    {
        ...
        // max_discount_amount is optional for both discount types
        // If not set, full calculated discount will be applied (no limit)
        ...
        if (!empty($invalidCourses)) {
            return ApiResponseService::validationError(
                'You can only create promo codes for your own courses. The following course IDs do not belong to you: ' .
        ...
        try {
            ...
            // Handle max_discount_amount: if not provided or empty, set to null (no limit)
            // null means no limit, any positive value means maximum discount cap
            if ($request->has('max_discount_amount') && 
                $request->max_discount_amount !== '' && 
                $request->max_discount_amount !== null &&
                is_numeric($request->max_discount_amount) && 
            ...
            // Handle minimum_order_amount: if not provided, set to 0 (no minimum)
            if (!isset($data['minimum_order_amount']) || $data['minimum_order_amount'] === '') {
...
public function getPromoCodesByInstructor(Request $request)
            ...
            if ($request->filled('id')) {
                ...
                $promoCode = PromoCode::where('id', $request->id)
                    ...
                    ->with(['courses' => function($q) {
                        $q->select('courses.id', 'courses.title', 'courses.slug', 'courses.price', 'courses.discount_price');
                    }])
                    ->withCount('courses')
                    ->first();

                if (!$promoCode) {
                    return ApiResponseService::validationError('Promo code not found or you do not have permission to view it');
                }
                ...
                $promoCodeData = [
                    ...
                    'is_active' => $promoCode->start_date <= now() && $promoCode->end_date >= now() && ($promoCode->no_of_users === null || $promoCode->no_of_users > 0),
                    'is_expired' => $promoCode->end_date < now(),
                    'is_upcoming' => $promoCode->start_date > now(),
                ...
                switch ($request->status) {
                    case 'active':
                        $query->where('start_date', '<=', $now)
                            ->where('end_date', '>=', $now)
                            ->where(function($q) {
                                $q->where('no_of_users', '>', 0)
                                  ->orWhereNull('no_of_users');
                            });
                        break;
                    case 'expired':
                    ...
                    case 'upcoming':
                        ...
                        break;
                    case 'used_up':
                        $query->whereNotNull('no_of_users')
                            ->where('no_of_users', '<=', 0);
                        break;
                }
            }
    ...
    public function updatePromoCodeByInstructor(Request $request, $id = null)
    {
        ...
        try {
            ...
            if (!$promoCode) {
                return ApiResponseService::validationError('Promo code not found or you do not have permission to update it');
            }
            ...
            if (!empty($invalidCourses)) {
                return ApiResponseService::validationError(
                    'You can only update promo codes for your own courses. The following course IDs do not belong to you: ' .
                    implode(', ', $invalidCourses)
                );
            }
    ...
    public function deletePromoCodeByInstructor(Request $request, $id = null)
    {
        ...
        try {
            ...
            if (!$promoCode) {
                return ApiResponseService::validationError('Promo code not found or you do not have permission to delete it');
            }
    ...
    public function getWalletDetails(Request $request)
    {
        try {
            ...
            $totalEarned = Commission::where('instructor_id', $user->id)
                ->where('status', 'paid')
                ->sum('instructor_commission_amount');
            
            $totalPending = Commission::where('instructor_id', $user->id)
                ->where('status', 'pending')
                ->sum('instructor_commission_amount');
    ...
    public function updateCourseStatus(Request $request)
    {
        try {
            ...
            if (!$course) {
                return ApiResponseService::errorResponse('Course not found or you do not have permission to update it.');
            }
    ...
    public function getWalletHistory(Request $request)
    {
        try {
            ...
            // Format the response
            $formattedHistory = $walletHistory->map(function($transaction) {
            ...
            // Calculate summary
            $totalCredit = \App\Models\WalletHistory::where('user_id', $instructorId)
                ->where('amount', '>', 0)
                ->sum('amount');
                
            $totalDebit = \App\Models\WalletHistory::where('user_id', $instructorId)
                ->where('amount', '<', 0)
                ->sum('amount');
    ...
    public function getAssignmentSubmissions(Request $request)
    {
        try {
            ...
            if ($request->filled('assignment_id') && $request->filled('assignment_slug')) {
                return ApiResponseService::validationError('Please provide either assignment_id or assignment_slug, not both');
            }
            ...
            ->whereHas('assignment.chapter.course', function($courseQuery) use ($instructorId) {
                ...
                $courseQuery->where(function($q) use ($instructorId) {
                    $q->where('user_id', $instructorId)
                      ->orWhereExists(function($subQuery) use ($instructorId) {
            ...
            // Filter by course
            if ($request->filled('course_id')) {
                $query->whereHas('assignment.chapter', function($chapterQuery) use ($request) {
            ...
            if ($request->filled('assignment_id')) {
                ...
                $assignment = \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::where('slug', $request->assignment_slug)->first();
                if ($assignment) {
            ...
            if ($request->filled('assignment_id') || $request->filled('assignment_slug')) {
                $assignment = null;
                if ($request->filled('assignment_id')) {
                    $assignment = \App\Models\Course\CourseChapter\Assignment\CourseChapterAssignment::with(['chapter.course'])
    ...
    public function updateAssignmentSubmission(Request $request)
    {
        try {
            ...
            $submission = UserAssignmentSubmission::with(['assignment.chapter.course'])
    ...
    public function getAssignmentSubmissionDetails(Request $request, $submissionId = null)
    {
        try {
            ...
            // Check if user is instructor
            if (!Auth::user()->hasRole('Instructor')) {
                return ApiResponseService::unauthorizedResponse("Only instructors can view assignment submission details.");
            }
            ...
            if (!$submission) {
                return ApiResponseService::validationError('Assignment submission not found or you do not have permission to view it');
            }
            ...
            return ApiResponseService::successResponse('Assignment submission details retrieved successfully', $response);
    ...
    public function getCategories(Request $request)
    {
        try {
            ...
            $buildTree = function($parentId = null) use (&$buildTree, $allCategories) {
                $result = [];
