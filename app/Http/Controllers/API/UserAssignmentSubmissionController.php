...
class UserAssignmentSubmissionController extends Controller
{
    ...
    public function getUserSubmissions(Request $request)
    {
        try {
            ...
            $query = UserAssignmentSubmission::where('user_id', $user->id)
                ->with(['assignment.chapter.course', 'files']);

            // Filter by course if provided
            if ($request->has('course_id') && $request->course_id) {
                $query->whereHas('assignment.chapter', function($q) use ($request) {
    ...
    public function getSubmissionDetails(Request $request, $id = null)
    {
        try {
            ...
            if (!$id) {
                ApiResponseService::validationError('Submission ID is required', ['id' => ['Submission ID is required']]);
            }

            // Validate request parameters
            $validator = Validator::make(['submission_id' => $id], [
    ...
    public function getCourseAssignments(Request $request, $courseId)
    {
        try {
            ...
            $assignments = CourseChapterAssignment::whereHas('chapter', function($query) use ($courseId) {
                    $query->where('course_id', $courseId);
                })
                ->where('is_active', true)
                ->with(['chapter', 'submissions' => function($query) use ($user) {
