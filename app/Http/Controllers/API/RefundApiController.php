...
class RefundApiController extends Controller
{
    ...
    public function requestRefund(Request $request)
    {
        ...
        try {
            ...
            $transaction = Transaction::whereHas('order', function($query) use ($user, $courseId) {
                $query->where('user_id', $user->id)
                      ->where('status', 'completed')
                      ->whereHas('orderCourses', function($subQuery) use ($courseId) {
            ...
            $orderCourse = OrderCourse::whereHas('order', function($query) use ($user) {
            ...
            if (Carbon::now()->greaterThan($refundDeadline)) {
                return ApiResponseService::validationError("Refund period has expired. You can only request refunds within {$refundPeriod} days of purchase");
            }
            ...
            if ($existingRefund) {
                return ApiResponseService::validationError("A refund request for this course is already pending or approved");
            }
            ...
            $refundAmount = $orderCourse->discount_price ?: $orderCourse->price;
        ...
        try {
            ...
            $refunds = RefundRequest::with([
                'course' => function($query) {
                    $query->select('id', 'title', 'thumbnail', 'user_id')->with(['user' => function($userQuery) {
            ...
            $refunds->getCollection()->transform(function ($refund) {
                ...
                $refund->admin_receipt_url = $refund->admin_receipt ? FileService::getFileUrl($refund->admin_receipt) : null;
                return $refund;
            });
    ...
    public function processRefund(Request $request)
    {
        ...
        try {
            ...
            if ($request->action === 'approve') {
                ...
                $refundRequest->user_media_url = $refundRequest->user_media ? FileService::getFileUrl($refundRequest->user_media) : null;
                $refundRequest->admin_receipt_url = null; // No receipt stored

                DB::commit();
                return ApiResponseService::successResponse("Refund approved and processed successfully", $refundRequest);
