...
class ReportsApiController extends Controller
{
    ...
    public function getCommissionReport(Request $request)
    {
        try {
            ...
            switch ($reportType) {
                case 'chart':
                    $data = $this->getCommissionChartData($query, $request);
                    break;
                case 'detailed':
                    $data = $this->getDetailedCommissionData($query, $request);
                    break;
                default:
                    $data = $this->getCommissionSummaryData($query, $request);
                    break;
            }
    ...
    public function getCourseReport(Request $request)
    {
        try {
            ...
            switch ($reportType) {
                case 'performance':
                    $data = $this->getCoursePerformanceData($query, $request);
                    break;
                case 'detailed':
                    $data = $this->getDetailedCourseData($query, $request);
                    break;
                default:
                    $data = $this->getCourseSummaryData($query, $request);
                    break;
            }
    ...
    public function getInstructorReport(Request $request)
    {
        try {
            ...
            switch ($reportType) {
                case 'performance':
                    $data = $this->getInstructorPerformanceData($query, $request);
                    break;
                case 'detailed':
                    $data = $this->getDetailedInstructorData($query, $request);
                    break;
                default:
                    $data = $this->getInstructorSummaryData($query, $request);
                    break;
            }
    ...
    public function getEnrollmentReport(Request $request)
    {
        try {
            ...
            switch ($reportType) {
                case 'chart':
                    $data = $this->getEnrollmentChartData($query, $request);
                    break;
                case 'detailed':
                    $data = $this->getDetailedEnrollmentData($query, $request);
                    break;
                default:
                    $data = $this->getEnrollmentSummaryData($query, $request);
                    break;
            }
        ...
        try {
            ...
            switch ($reportType) {
                ...
                case 'chart':
                    ...
                    break;
                case 'detailed':
                    $data = $this->getDetailedRevenueData($request);
                    break;
                default:
    ...
    {
        $perPage = $request->per_page ?? 15;
        return $query->withCount(['orderCourses', 'ratings'])
    ...
    private function getCoursePerformanceData($query, $request)
    {
        return $query->withCount(['orderCourses', 'ratings'])
                    ->withAvg('ratings', 'rating')
                    ->with(['orderCourses' => function($q) {
                        $q->selectRaw('course_id, SUM(price) as total_revenue, COUNT(*) as enrollment_count')
                          ->groupBy('course_id');
                    }])
                    ->get()
                    ->map(function($course) {
    ...
    private function getInstructorSummaryData($query, $request)
    {
        ...
        return [
            ...
            'total_courses_created' => $instructors->sum(function($instructor) {
                return $instructor->user->courses->count();
            }),
    ...
    {
        $perPage = $request->per_page ?? 15;
        return $query->with(['user.courses' => function($q) {
    ...
    {
        return $query->with(['user.courses.orderCourses', 'user.courses.ratings'])
                    ->get()
                    ->map(function($instructor) {
                        ...
                        return [
                            ...
                            'performance_metrics' => [
                                'total_courses' => $courses->count(),
                                'total_enrollments' => $courses->sum(function($course) {
                                    return $course->orderCourses->count();
                                }),
                                'total_revenue' => $courses->sum(function($course) {
                                    return $course->orderCourses->sum('price');
                                }),
    ...
    private function getRevenueSummaryData($request)
    {
        ...
        return [
            ...
            'total_orders' => $orders->count(),
            'average_order_value' => $orders->avg('final_price'),
            'revenue_by_payment_method' => $orders->groupBy('payment_method')->map(function($orders) {
    ...
    private function getRevenueComparisonData($request)
    {
        ...
        // Current period
        $currentQuery = Order::where('status', 'completed')
    ...
    {
        return $orders->flatMap->orderCourses
                     ->groupBy('course_id')
                     ->map(function($orderCourses) {
                         return [
    ...
    {
        return $commissions->groupBy('instructor_id')
                          ->map(function($instructorCommissions) {
                              return [
    ...
    {
        return $commissions->groupBy('course_id')
                          ->map(function($courseCommissions) {
                              return [
    ...
    {
        return $enrollments->groupBy('course_id')
                          ->map(function($courseEnrollments) {
                              return [
    ...
    private function getRevenueTrend($orders)
    {
        return $orders->groupBy(function($order) {
                     ...
                     ->map(function($dailyOrders) {
                         return $dailyOrders->sum('final_price');
                     })
                     ->sortKeys();
    }
