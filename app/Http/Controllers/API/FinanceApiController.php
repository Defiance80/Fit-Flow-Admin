...
class FinanceApiController extends Controller
{
    ...
    public function getInstructorEarnings(Request $request)
    {
        try {
            ...
            $earnings = $query->groupBy('instructor_id')
                ->orderBy('total_earnings', 'desc')
                ->get();
    ...
    public function createWithdrawalRequest(Request $request)
    {
        try {
            ...
            if (!$instructor) {
                return ApiResponseService::errorResponse('Only approved instructors can create withdrawal requests.', [], 403);
            }
    ...
    public function getWithdrawalRequests(Request $request)
    {
        try {
            ...
            if (!$instructor) {
                return ApiResponseService::errorResponse('Only approved instructors can view withdrawal requests.', [], 403);
            }
    ...
    public function getWalletSummary(Request $request)
    {
        try {
            ...
            // Get total earnings
            $totalEarnings = Commission::where('instructor_id', $user->id)
                ->where('status', 'paid')
                ->sum('instructor_commission_amount');

            // Get pending earnings
            $pendingEarnings = Commission::where('instructor_id', $user->id)
    ...
    public function getWithdrawalRequestDetails(Request $request)
    {
        try {
            ...
            $response = [
                ...
                'wallet_history' => $walletHistory->map(function($transaction) {
                    return [
                        'id' => $transaction->id,
                        'amount' => $transaction->amount,
                        'type' => $transaction->type,
