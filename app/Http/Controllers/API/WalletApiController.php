...
class WalletApiController extends Controller
{
    ...
    public function getWalletSummary(Request $request)
    {
        try {
            ...
            $totalCredits = WalletHistory::where('user_id', $user->id)
                ->where('type', 'credit')
                ->sum('amount');

            // Get total debits (sum of all debit transactions)
            $totalDebits = WalletHistory::where('user_id', $user->id)
                ->where('type', 'debit')
                ->sum('amount');
    ...
    public function getWalletHistory(Request $request)
    {
        try {
            ...
            $allTransactionsQuery = $query->with([
            ...
            $allTransactionsQuery->transform(function($transaction) {
                $courseName = null;
                $transactionId = null;
                $orderNumber = null;
                $status = null;
                ...
                if ($transaction->reference) {
                    ...
                    elseif ($referenceType === 'App\Models\RefundRequest') {
                    ...
                    elseif ($referenceType === 'App\Models\WithdrawalRequest') {
                        $status = $reference->status;
                        $paymentMethod = $reference->payment_method;
                        $paymentDetails = $reference->payment_details;
                    }
                    // Handle Commission reference
                    elseif ($referenceType === 'App\Models\Commission') {
                        ...
                        if ($reference->order) {
                            ...
                            $orderTransaction = \App\Models\Transaction::where('order_id', $reference->order->id)->first();
                            if ($orderTransaction) {
                                $transactionId = $orderTransaction->transaction_id;
                            }
                        }
            ...
            $pendingWithdrawalEntries = $pendingWithdrawals->map(function($withdrawal) use ($user) {
            ...
            $transactionsArray = $allTransactionsQuery->map(function($transaction) {
            ...
            $newTotal = $originalTotal + $pendingCount;
    ...
    public function createWithdrawalRequest(Request $request)
    {
        try {
            ...
            if (empty($paymentDetailsInput) || !is_array($paymentDetailsInput)) {
                return ApiResponseService::validationError('The payment details field is required. Please provide payment_details as an array or nested parameters like payment_details[account_holder_name].');
            }
            ...
            if ($user->wallet_balance < $amount) {
                return ApiResponseService::validationError('Insufficient wallet balance. Available: ' . number_format($user->wallet_balance, 2));
            }
            ...
            if ($pendingRequest) {
                return ApiResponseService::validationError('You already have a pending withdrawal request. Please wait for it to be processed.');
            }
    ...
    public function getWithdrawalRequests(Request $request)
    {
        try {
            ...
            $query = WithdrawalRequest::where('user_id', $user->id)
                ->where(function($q) {
                    $q->where('entry_type', 'user')
                      ->orWhereNull('entry_type'); // Include old records without entry_type (treat as user)
                });
            ...
            // Format the response
            $formattedRequests = $withdrawalRequests->map(function($request) {
                return [
                    'id' => $request->id,
                    ...
                    'processed_at_formatted' => $request->processed_at ? $request->processed_at->format('Y-m-d H:i:s') : null,
                ];
            });
    ...
    public function getWithdrawalRequestDetails(Request $request)
    {
        try {
            ...
            if ($withdrawalRequest->entry_type !== 'user' && !is_null($withdrawalRequest->entry_type)) {
                return ApiResponseService::errorResponse('This withdrawal request is not a user-side request.', [], 403);
            }

            // Get wallet history related to this withdrawal
            $walletHistory = WalletHistory::where('user_id', $user->id)
                ->where('reference_type', 'App\Models\WithdrawalRequest')
            ...
            $response = [
                'withdrawal_request' => [
                    ...
                    'amount' => (float) $withdrawalRequest->amount,
                    'status' => $withdrawalRequest->status,
                    'status_label' => ucfirst($withdrawalRequest->status),
                    ...
                    'processed_at_formatted' => $withdrawalRequest->processed_at ? $withdrawalRequest->processed_at->format('Y-m-d H:i:s') : null,
                ],
            ...
            return ApiResponseService::errorResponse('Failed to retrieve withdrawal request details: ' . $e->getMessage());
        }
    }
