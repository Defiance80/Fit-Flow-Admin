...
class LanguageController extends Controller {
    ...
    public function show(Request $request)
    {
        ...
        foreach ($result as $key => $row) {
            ...
            $operate .= BootstrapTableService::button('fa fa-edit', route('language.edit', $row->id), ['btn-primary'], ['title' => 'Edit']);
            // Only show delete button if language is not default
            if (!$row->is_default) {
                $operate .= BootstrapTableService::deleteButton(route('language.destroy',$row->id),null,null,null,' delete-language');
            }
            $dropdownItems = [
                [
                    'icon' => '',
