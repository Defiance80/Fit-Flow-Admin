...
class FeatureSectionController extends Controller
{
    public function index()
    {
       ...
       // ResponseService::noAnyPermissionThenRedirect(['tax-list', 'tax-create', 'tax-edit', 'tax-delete']);
    ...
    public function show(Request $request, $id)
    {
        ...
        foreach ($result as $row) {
            if ($showDeleted) {
                ...
                $operate = BootstrapTableService::editButton(route('feature-sections.update', $row->id), true, '#featureSectionEditModal', $row->id);
                $operate .= BootstrapTableService::deleteButton(route('feature-sections.destroy', $row->id));
            }

            $tempRow = $row->toArray();
            $tempRow['no'] = $no++;
            // Format type value: convert underscore-separated to title case
            // e.g., "top_rated_courses" -> "Top Rated Courses"
            if (!empty($tempRow['type'])) {
                $tempRow['type'] = ucwords(str_replace('_', ' ', $tempRow['type']));
            }
            // Get image URLs properly
            $tempRow['images'] = $row->images->map(function($image) {
