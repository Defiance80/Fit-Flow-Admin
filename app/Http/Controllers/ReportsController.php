...
class ReportsController extends Controller
{
    ...
    public function getReportFilters(Request $request)
    {
        try {
            $data = [
                'courses' => Course::select('id', 'title')->where('is_active', true)->get(),
    ...
    public function getSalesReportData(Request $request)
    {
        try {
            ...
            switch ($reportType) {
                case 'chart':
                    $data = $this->getSalesChartData($query, $request);
                    break;
                case 'detailed':
                    $data = $this->getDetailedSalesData($query, $request);
                    break;
                default:
                    $data = $this->getSalesSummaryData($query, $request);
                    break;
            }
    ...
    public function instructor()
    {
        ...
        if (\App\Services\InstructorModeService::isSingleInstructorMode()) {
            return redirect()->route('dashboard')->with('info', 'Instructor reports are disabled in Single Instructor mode.');
        }
    ...
    public function getCommissionReportData(Request $request)
    {
        try {
            ...
            $topEarningInstructors = $commissions->groupBy('instructor_id')
                ->map(function ($group) {
            ...
            $commissionByCourse = $commissions->groupBy('course_id')
                ->map(function ($group) {
                    $course = $group->first()->course;
                    return [
                        'course' => $course,
                        'total_commission' => $group->sum('admin_commission_amount') + $group->sum('instructor_commission_amount'),
                        'commission_count' => $group->count(),
    ...
    public function getCourseReportData(Request $request)
    {
        try {
            ...
            if ($reportType === 'performance') {
                ...
                $performanceData = $courses->map(function($course) {
                    return [
                        'course' => $course,
                        'performance_metrics' => [
            ...
            $totalEnrollments = $courses->sum(function($course) {
                if ($course->orderCourses) {
                    return $course->orderCourses->filter(function($orderCourse) {
                        return $orderCourse->order && $orderCourse->order->status === 'completed';
                    })->count();
                }
                return 0;
            });
            ...
            $coursesWithRatings = $courses->filter(function($course) {
                return $course->ratings && $course->ratings->count() > 0;
            });
            
            $averageRating = 0;
            if ($coursesWithRatings->count() > 0) {
                $totalRating = $coursesWithRatings->sum(function($course) {
                    return $course->ratings->avg('rating') ?: 0;
                });
                $averageRating = $totalRating / $coursesWithRatings->count();
            }

            // Group courses by category for chart
            $coursesByCategory = $courses->groupBy('category_id')->map(function($group, $categoryId) {
            ...
            $coursesByLevel = $courses->groupBy('level')->map(function($group, $level) {
                return [
                    'level' => ucfirst($level ?: 'Not Specified'),
                    'count' => $group->count()
                ];
            })->values();
    ...
    {
        if ($request->filled('instructor_id')) {
            $query->whereHas('orderCourses.course', function($q) use ($request) {
    ...
    {
        if ($request->filled('category_id')) {
            $query->whereHas('orderCourses.course.category', function($q) use ($request) {
    ...
    private function getSalesSummaryData($query, $request)
    {
        ...
        if ($orders->isNotEmpty()) {
            $topCourses = $orders->flatMap(function($order) {
                if ($order->orderCourses && $order->orderCourses->isNotEmpty()) {
                    return $order->orderCourses->map(function($orderCourse) use ($order) {
                        return [
            ...
            })->groupBy('course.id')->map(function($group) {
                $course = $group->first()['course'];
                return [
                    'course' => $course,
        ...
        return [
            ...
            'recent_orders' => $orders->isNotEmpty() ? $orders->sortByDesc('created_at')->take(10)->values() : collect()
        ];
    }
    ...
    public function exportCourseReport(Request $request)
    {
        try {
            ...
            $courses = $courses->map(function($course) {
                ...
                $course->revenue = 0;
                if ($course->orderCourses) {
                    $course->revenue = $course->orderCourses->filter(function($orderCourse) {
                        return $orderCourse->order && $orderCourse->order->status === 'completed';
                    })->sum(function($orderCourse) {
    ...
    private function exportCourseExcel($courses, $request)
    {
        ...
        return response()->stream(function() use ($courses, $currencySymbol) {
    ...
    private function cleanHtmlForPdf($html)
    {
        ...
        if (function_exists('iconv')) {
            $cleaned = @iconv('UTF-8', 'UTF-8//IGNORE', $html);
            if ($cleaned !== false && $cleaned !== '') {
                $html = $cleaned;
            }
        }
    ...
    public function getInstructorReportData(Request $request)
    {
        try {
            ...
            $query = User::whereHas('instructor_details')
                ->with(['instructor_details', 'courses.orderCourses.order']);
            ...
            if ($request->filled('status')) {
                $query->whereHas('instructor_details', function($q) use ($request) {
            ...
            $instructors = $instructors->map(function($instructor) {
                ...
                if ($instructor->courses) {
                    $instructor->total_enrollments = $instructor->courses->sum(function($course) {
                        return $course->orderCourses ? $course->orderCourses->count() : 0;
                    });
                }
                ...
                if ($instructor->courses) {
                    $instructor->total_revenue = $instructor->courses->sum(function($course) {
                        if ($course->orderCourses) {
                            return $course->orderCourses->filter(function($orderCourse) {
                                return $orderCourse->order && $orderCourse->order->status === 'completed';
                            })->sum(function($orderCourse) {
            ...
            $data = [
                ...
                'approved_instructors' => $instructors->filter(function($instructor) {
                    return $instructor->instructor_details->status === 'approved';
                })->count(),
                'individual_instructors' => $instructors->filter(function($instructor) {
                    return $instructor->instructor_details->type === 'individual';
                })->count(),
    ...
    private function exportInstructorExcel($instructors, $request)
    {
        ...
        return response()->stream(function() use ($instructors, $currencySymbol) {
            $file = fopen('php://output', 'w');
    ...
    public function getEnrollmentReportData(Request $request)
    {
        try {
            ...
            // Group by course
            $enrollmentsByCourse = $allEnrollments->groupBy('course_id')->map(function($group) {
                $course = $group->first()->course;
                return [
                    'course' => $course,
            ...
            // Group by month
            $enrollmentsByMonth = $allEnrollments->groupBy(function($enrollment) {
            ...
            $perPage = $request->get('per_page', 15);
            $page = $request->get('page', 1);
            $paginatedEnrollments = $query->orderBy('created_at', 'desc')
                ->paginate($perPage, ['*'], 'page', $page);

            // Calculate progress for each enrollment
            $enrollments = $paginatedEnrollments->map(function($enrollment) {
                ...
                if ($enrollment->course && $enrollment->course->chapters) {
                    ...
                    if ($totalChapters > 0) {
                        $completedChapters = \App\Models\Course\UserCourseChapterTrack::where('user_id', $enrollment->user_id)
                            ->whereHas('chapter', function($q) use ($enrollment) {
            ...
            $sanitizedEnrollments = $enrollments->map(function($enrollment) {
                ...
                if ($enrollment->course) {
                    ...
                    if ($enrollment->course->category) {
                        $enrollment->course->category->name = $this->sanitizeUtf8($enrollment->course->category->name ?? 'N/A');
                    }
                }
                return $enrollment;
            });
    ...
    private function getRevenueSummaryData(Request $request)
    {
        ...
        $revenueByPaymentMethod = $orders->groupBy('payment_method')->map(function ($orders) {
            return $orders->sum('final_price');
        })->toArray();
        ...
        $topRevenueCourses = collect($courseRevenue)
            ->sortByDesc('revenue')
            ->values()
            ->take(5)
            ->toArray();
    ...
    private function exportRevenueExcel($orders, $request)
    {
        ...
        return response()->stream(function() use ($orders, $currencySymbol) {
            $file = fopen('php://output', 'w');
    ...
    private function exportRevenuePDF($orders, $request)
    {
        try {
            ...
            $filters = array_filter($request->only([
                'date_from', 'date_to', 'course_id', 'instructor_id', 
