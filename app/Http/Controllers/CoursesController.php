...
class CoursesController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('courses.index', compact('categories', 'tags', 'course_languages', 'instructors'), ['type_menu' => 'courses']);
    }
    ...
    public function create()
    {
        ...
        return view('courses.create', compact('categories', 'tags', 'course_languages', 'instructors', 'maxVideoSizeMB'), ['type_menu' => 'courses']);
    }
    ...
    // ResponseService::noPermissionThenSendJson('courses-create');
        ...
        if ($request->course_type === 'paid' && $request->has('price') && $request->has('discount_price')) {
            ...
            $discountPrice = round($request->discount_price, 2);
        ...
        try {
            ...
            $price = $request->has('price') ? round($request->price, 2) : null;
            $discountPrice = $request->has('discount_price') ? round($request->discount_price, 2) : null;
            ...
            if ($request->hasFile('intro_video')) {
                $data['intro_video'] =  FileService::compressAndUpload($request->file('intro_video'), $this->videoUploadFolder);
            }

            // Upload Meta Image
            if ($request->hasFile('meta_image')) {
                $data['meta_image'] = FileService::compressAndUpload($request->file('meta_image'), $this->metaImageUploadFolder);
            }
    ...
    public function show()
    {
        ...
        $sql = Course::with(['user', 'category', 'learnings', 'requirements', 'tags', 'language'])
            // Only show approved courses by default
        ...
        ->when(isset($filterIsActive) && $filterIsActive !== '', function ($query) use ($filterIsActive) {
            $query->where('is_active', (bool)$filterIsActive);
        })
        ->when(!empty($filterInstructorId), function ($query) use ($filterInstructorId) {
    ...
    public function requestsList()
    {
        ...
        $sql = Course::with(['user','category'])
            ...
            ->when(!empty($instructorId), function ($query) use ($instructorId) {
    ...
    public function view($id)
    {
        ...
        $course = Course::with([
            ...
            'chapters.lectures.resources' => function($query) {
            ...
            'chapters.quizzes.questions' => function($query) {
                // Load quiz questions
                $query->orderBy('order', 'asc');
            },
            'chapters.quizzes.questions.options' => function($query) {
    ...
    public function edit($id)
    {
        ...
        return view('courses.edit', compact('course', 'categories', 'parent_category_data', 'parent_category', 'tags', 'course_languages', 'instructors', 'shouldShowInstructorFilters', 'maxVideoSizeMB'), ['type_menu' => 'courses']);
    }
    /**
    ...
    {
        ResponseService::noPermissionThenSendJson('courses-edit');
        $idValidator = Validator::make(['id' => $request->id], [
            'id' => 'required|exists:courses,id',
        ],[
        ...
        try {
            ...
            $data['sequential_access'] = $request->has('sequential_access') ? ($request->sequential_access ? 1 : 0) : ($course->sequential_access ?? 0);
            $data['certificate_enabled'] = $request->certificate_enabled ?? 0; // Set Certificate Enabled (default: false)
            $data['certificate_fee'] = $request->certificate_enabled ? ($request->certificate_fee ?? 0) : null; // Set Certificate Fee
            ...
            if (!$isAdmin && !$isCourseOwner && !$isTeamMember) {
                return ResponseService::validationError('You are not authorized to update this course');
            }

            // Delete old thumbnail and upload new one
            if ($request->hasFile('thumbnail')) {
                $data['thumbnail'] = FileService::compressAndReplace($request->file('thumbnail'), $this->uploadFolder, $course->thumbnail);
            }

            // Delete old intro video and upload new one
            if ($request->hasFile('intro_video')) {
                $data['intro_video'] = FileService::compressAndReplace($request->file('intro_video'), $this->videoUploadFolder, $course->intro_video);
            }

            // Delete old meta image and upload new one
            if ($request->hasFile('meta_image')) {
                $data['meta_image'] = FileService::compressAndReplace($request->file('meta_image'), $this->metaImageUploadFolder, $course->meta_image);
            }
            ...
            if ($isAdmin) {
                ...
                $data['status'] = in_array($requestedStatus, ['draft','publish']) ? $requestedStatus : ($course->status ?? 'draft');
                ...
                // For admins, respect the explicit is_active toggle
                $data['is_active'] = $request->has('is_active') ? ($request->is_active ? 1 : 0) : ($course->is_active ?? 0);
            } elseif ($isInstructor) {
                // Instructor cannot approve; publish request becomes pending
                if ($requestedStatus === 'publish') {
                    $data['status'] = 'pending';
                } elseif ($requestedStatus === 'draft') {
                    $data['status'] = 'draft';
                } else {
                    $data['status'] = $course->status; // keep existing if not explicitly set
                }
                $data['approval_status'] = null;
                // For instructors, derive is_active from status and approval
                $data['is_active'] = ($data['status'] === 'publish' && ($data['approval_status'] ?? null) === 'approved') ? 1 : 0;
            } else {
    ...
    public function destroy($id)
    {
        ...
        try {
            ...
            if (
                ...
                !Auth::user()->hasAnyRole(['admin', 'instructor'])
            ) {
                return ResponseService::validationError('You are not authorized to delete this course');
            }
    ...
    public function languagesIndex(){
        ResponseService::noAnyPermissionThenRedirect(['course-languages-list','course-languages-create','course-languages-edit','course-languages-delete']);
    ...
    public function languagesList(Request $request){
        ...
        $sql = CourseLanguage::query()
            ...
            ->when($showDeleted == '1', function ($query) {
                $query->onlyTrashed();
            });
        ...
        foreach ($res as $row) {
            if ($showDeleted) {
                ...
                $operate = BootstrapTableService::editButton(route('courses.languages.edit', $row->id),true,'#languageEditModal',$row->id);
    ...
    {
        ResponseService::noAnyPermissionThenRedirect(['course-tags-list', 'course-tags-create', 'course-tags-edit', 'course-tags-delete']);
    ...
    public function tagList()
    {
        ...
        foreach ($res as $row) {
            if ($showDeleted) {
                ...
                $operate = BootstrapTableService::editButton(route('tags.edit', $row->id), true, '#tagEditModal', $row->id);
