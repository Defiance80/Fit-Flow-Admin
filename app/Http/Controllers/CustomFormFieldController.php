...
class CustomFormFieldController extends Controller
{
    ...
    {
        ResponseService::noAnyPermissionThenRedirect(['custom-form-fields-list','custom-form-fields-create','custom-form-fields-edit','custom-form-fields-delete']);
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (in_array($request->type, ['dropdown', 'radio', 'checkbox']) && $request->has('default_data')) {
                ...
                $optionsCount = count(array_filter($request->default_data, function($option) {
                ...
                if (in_array($request->type, ['dropdown', 'radio']) && $optionsCount < 2) {
                    ...
                    return ResponseService::validationError('At least 2 options are required for ' . ucfirst($request->type) . ' type.');
                }
    ...
    public function show(Request $request)
    {
        ...
        foreach ($res as $row) {
            $operate = $showDeleted
                ? BootstrapTableService::restoreButton(route('custom-form-fields.restore', $row->id)) .
                BootstrapTableService::trashButton(route('custom-form-fields.trash', $row->id))
                : BootstrapTableService::editButton(route('custom-form-fields.edit', $row->id)) .
                BootstrapTableService::deleteButton(route('custom-form-fields.destroy', $row->id));
            ...
            if (!empty($tempRow['type'])) {
                $tempRow['type'] = ucfirst($tempRow['type']);
            }
    ...
    public function edit($id)
    {
        ...
        $defaultValues = $customField->options->map(function ($option) {
            return [
    ...
    public function update(Request $request, CustomFormField $customFormField)
    {
        ...
        try {
            ...
            if (in_array($request->type, ['dropdown', 'radio', 'checkbox']) && $request->has('edit_default_data')) {
                ...
                // Validate minimum 2 options for radio and dropdown types
                $optionsCount = count(array_filter($request->edit_default_data, function($option) {
                    return !empty($option['option']);
                }));
