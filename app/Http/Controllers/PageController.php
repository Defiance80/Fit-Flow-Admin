...
class PageController extends Controller
{
        ...
        try {
            ...
            if (!$isCustomPage) {
                ...
                if ($existingPage) {
                    ...
                    return ResponseService::validationError("A {$pageTypeName} page already exists for this language. Only one {$pageTypeName} page is allowed per language.");
                }
            }
            
            // Generate unique slug if not provided or if it already exists for this language
            if (empty($data['slug']) || Page::where('slug', $data['slug'])->where('language_id', $data['language_id'])->exists()) {
            ...
            if ($request->hasFile('page_icon')) {
                ...
                $iconName = 'page_icon_' . Str::slug($data['title']) . '_' . time() . '.' . $icon->getClientOriginalExtension();
            ...
            if ($request->hasFile('og_image')) {
                ...
                $ogImageName = 'og_image_' . Str::slug($data['title']) . '_' . time() . '.' . $ogImage->getClientOriginalExtension();
    ...
    public function show(Request $request)
    {
        ...
        foreach ($res as $row) {
            if ($showDeleted) {
                ...
                if ($row->is_custom == 1 || strtolower($row->page_type) == 'custom') {
                    $operate .= BootstrapTableService::trashButton(route('pages.trash', $row->id));
                }
            } else {
                $operate = BootstrapTableService::editButton(route('pages.update', $row->id), true, '#pageEditModal', $row->id);
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            if ($existingPageBySlug) {
                ...
                $slug = $baseSlug;
                $counter = 1;
                while (Page::where('slug', $slug)->where('language_id', $data['language_id'])->where('id', '!=', $id)->exists()) {
                    $slug = $baseSlug . '-' . $counter;
                    $counter++;
                }
                $data['slug'] = $slug;
    ...
    public function destroy($id)
    {
        try {
            ...
            if ($page->is_custom != 1 && strtolower($page->page_type) != 'custom') {
                DB::rollBack();
