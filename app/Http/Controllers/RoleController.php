...
class RoleController extends Controller
{
    ...
    public function create()
    {
        ...
        foreach ($permission as $key => $val) {
            $lastDashPos = strrpos($val->name, "-");
            if ($lastDashPos !== false) {
                $subArr = substr($val->name, 0, $lastDashPos);
            ...
            if (isset($permissionArray['name']) && !is_scalar($permissionArray['name'])) {
                $permissionArray['name'] = is_array($permissionArray['name']) ? json_encode($permissionArray['name']) : (string)$permissionArray['name'];
            }
