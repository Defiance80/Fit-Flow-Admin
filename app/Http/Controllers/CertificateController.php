...
class CertificateController extends Controller
{
    ...
    public function download(Request $request)
    {
        ...
        if (!$this->isCourseCompleted($user->id, $course_id)) {
            return ApiResponseService::validationError("Course not completed. Please complete all lessons, quizzes, and assignments to generate certificate.");
        }
    ...
    private function generateCertificateHtml($template, $certificate)
{
    ...
    if (isset($settings['elements']) && is_array($settings['elements'])) {
        foreach ($settings['elements'] as $element) {
            ...
            if (isset($styles['fontSize'])) $styleString .= "font-size:{$styles['fontSize']};";
            if (isset($styles['color'])) $styleString .= "color:{$styles['color']};";
            if (isset($styles['fontWeight'])) $styleString .= "font-weight:{$styles['fontWeight']};";
            if (isset($styles['fontFamily'])) $styleString .= "font-family:{$styles['fontFamily']};";
            if (isset($styles['textAlign'])) $styleString .= "text-align:{$styles['textAlign']};";
            ...
            if (($element['type'] ?? '') === 'image') {
                ...
                $imgSrc = strpos($element['content'], 'http') === 0
                    ? $element['content']
    ...
    if ($template->signature_image) {
        ...
        $html .= '
            <div style="position:absolute;
                        ...
                        width:'.$sigWidth.'px;
                        height:'.$sigHeight.'px;">
                <img src="'.asset('storage/'.$template->signature_image).'" 
                     style="width:100%;
                            height:100%;
                            object-fit:contain;">
            </div>';
    ...
    private function isCourseCompleted($user_id, $course_id): bool
    {
        ...
        if (!empty($assignmentIds)) {
            $submittedAssignments = \App\Models\Course\CourseChapter\Assignment\UserAssignmentSubmission::where('user_id', $user_id)
    ...
    public function download_quiz_certificate($certificate_id)
    {
        ...
        $certificate = \App\Models\QuizCertificate::with(['user', 'attempt.quiz'])
            ->find($certificate_id);
