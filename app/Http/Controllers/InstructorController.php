...
class InstructorController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        foreach ($res as $row) {
            $operate = BootstrapTableService::button('fas fa-check','', ['btn-info','change-status'],array('title' => 'Change Status', "data-target" => "#instructorEditModal", "data-toggle" => "modal", "data-id" => $row->id));
            $operate .= BootstrapTableService::button('fas fa-eye', route('instructor.show-form',$row->id), ["btn","icon","btn-primary","view-form-btn"],array('title' => 'View Form'));
    ...
    public function updateStatus(Request $request, $id)
    {
        ...
        try {
            ...
            $instructor->reason = ($data['status'] == 'rejected' || $data['status'] == 'suspended') ? $data['reason'] : null;
            $instructor->save();
            ...
            if (($data['status'] == 'approved' || $data['status'] == 'rejected' || $data['status'] == 'suspended') && $oldStatus != $data['status']) {
                $user->notify(new \App\Notifications\InstructorStatusUpdateNotification($instructor, $data['status'], $data['reason'] ?? null));
            }
    ...
    public function showForm($id){
        ...
        ResponseService::noPermissionThenSendJson('instructors-list');
        $instructor = Instructor::with(['user','personal_details','social_medias' => function($socialMediaQuery){
            $socialMediaQuery->with('social_media:id,name');
        },'other_details' => function($otherDetailQuery){
    ...
    public function walletHistory()
    {
        ...
        if (InstructorModeService::isSingleInstructorMode()) {
            return redirect()->route('dashboard')->with('info', 'Instructor wallet history is disabled in Single Instructor mode.');
        }
    ...
    public function getWalletHistoryData(Request $request)
    {
        ...
        $sql = \App\Models\WalletHistory::with(['user'])
            ...
            ->when(!empty($transactionType), function ($q) use ($transactionType) {
        ...
        foreach ($result as $row) {
            ...
            $entryTypeBadge = $row->entry_type === 'instructor' ? 'warning' : ($row->entry_type === 'user' ? 'primary' : 'info');
            ...
            $tempRow = [
                ...
                'amount' => '<span class="' . $amountClass . '">' . $amountPrefix . '₹' . number_format(abs($row->amount), 2) . '</span>',
    ...
    public function withdrawalRequests()
    {
        ...
        // In single instructor mode, redirect to dashboard
        if (InstructorModeService::isSingleInstructorMode()) {
            return redirect()->route('dashboard')->with('info', 'Instructor withdrawal requests are disabled in Single Instructor mode.');
        }
        ...
        // Get withdrawal requests count for debugging
        $withdrawalCount = WithdrawalRequest::count();
        
        return view('instructor.withdrawal-requests', compact('type_menu', 'instructors'));
    }
    ...
    private function getWithdrawalRequestDetails(Request $request)
    {
        ...
        $withdrawalRequest = WithdrawalRequest::with(['user:id,name,email', 'processedBy:id,name'])
        ...
        $data = [
            ...
            'processed_at' => $withdrawalRequest->processed_at ? $withdrawalRequest->processed_at->format('d M Y, h:i A') : null,
    ...
    private function getActionButtons($row)
    {
        ...
        // View details button
        $buttons .= '<button class="btn btn-sm btn-info view-details" data-id="' . $row->id . '" title="View Details">
                        <i class="fas fa-eye"></i>
                    </button> ';

        // Status update buttons based on current status
        if ($row->status === 'pending') {
            $buttons .= '<button class="btn btn-sm btn-success approve-request" data-id="' . $row->id . '" title="Approve">
                            <i class="fas fa-check"></i>
                        </button> ';
            $buttons .= '<button class="btn btn-sm btn-danger reject-request" data-id="' . $row->id . '" title="Reject">
                            <i class="fas fa-times"></i>
                        </button> ';
        } elseif ($row->status === 'approved') {
            $buttons .= '<button class="btn btn-sm btn-info process-request" data-id="' . $row->id . '" title="Mark as Processing">
                            <i class="fas fa-cog"></i>
                        </button> ';
        } elseif ($row->status === 'processing') {
            $buttons .= '<button class="btn btn-sm btn-primary complete-request" data-id="' . $row->id . '" title="Mark as Completed">
