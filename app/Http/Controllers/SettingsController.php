...
class SettingsController extends Controller
{
    ...
    public function system()
    {
        ...
        $systemVersion = Setting::firstOrCreate(
            ['name' => 'system_version'],
            ['value' => '1.0.0', 'type' => 'string']
        );
        
        $settings = HelperService::systemSettings(array('app_name', 'website_url', 'announcement_bar', 'favicon', 'vertical_logo', 'horizontal_logo', 'placeholder_image', 'login_banner_image', 'contact_address', 'contact_email', 'contact_phone', 'system_color', 'system_light_color', 'hover_color', 'footer_description', 'schema', 'currency_code','currency_symbol','instructor_mode','individual_admin_commission','team_admin_commission','tax_type','max_video_upload_size','maintaince_mode','system_version','refund_enabled','refund_period_days','refund_policy'));
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $settings = HelperService::systemSettings(array('favicon', 'vertical_logo', 'horizontal_logo', 'placeholder_image', 'login_banner_image','system_color','rgb_color'));
            ...
            foreach ($data as $name => $value) {
                ...
                $settingsData[] = array('name' => $name, 'value' => $value, 'type' => $type);
            }
            ...
            if(!empty($request->social_media_data)) {
                foreach($request->social_media_data as $socialMedia) {
                    ...
                    if(isset($socialMedia['icon']) && is_file($socialMedia['icon'])) {
                        $iconPath = FileService::compressAndReplace($socialMedia['icon'], $this->socialMediaFolder, $iconExists);
                    }
            ...
            HelperService::systemSettings(['currency_symbol', 'currency_code', 'tax_type']);
    ...
    public function firebase()
    {
        ...
        $settings = HelperService::systemSettings(array('firebase_api_key', 'firebase_auth_domain', 'firebase_project_id', 'firebase_storage_bucket', 'firebase_messaging_sender_id', 'firebase_app_id', 'firebase_measurement_id', 'firebase_service_file'));
    ...
    public function updateFirebase(Request $request)
    {
        ...
        try {
            ...
            $settingArray = [];
            foreach ($data as $name => $value) {
                $settingArray[] = array('name' => $name, 'value' => $value, 'type' => 'text');
            }
            ...
            if ($request->hasFile('firebase_service_file')) {
                ...
                $settingArray[] = array('name' => 'firebase_service_file', 'value' => $path, 'type' => 'file');
            }
    ...
    {
        $settings = HelperService::systemSettings(array('refund_policy', 'refund_period_days', 'refund_enabled'));
    ...
    public function updateRefund(Request $request)
    {
        ...
        try {
            ...
            foreach ($data as $name => $value) {
                ...
                $settingArray[] = array('name' => $name, 'value' => $value, 'type' => $type);
            }
            Setting::upsert($settingArray, ['name']);
    ...
    {
        $settings = HelperService::systemSettings(['playstore_url', 'appstore_url', 'android_version', 'ios_version', 'app_version', 'force_update']);
    ...
    public function paymentGateway()
    {
        ...
        $razorpayPaymentGateway = HelperService::systemSettings(['razorpay_status', 'razorpay_api_key', 'razorpay_secret_key', 'razorpay_webhook_url', 'razorpay_webhook_secret_key']);
    ...
    public function updatePaymentGateway(Request $request)
    {
        try {
            ...
            $razorpayEnabled = $request->has('razorpay_status') && ($request->razorpay_status == 1 || $request->razorpay_status == '1');
            $stripeEnabled = $request->has('stripe_status') && ($request->stripe_status == 1 || $request->stripe_status == '1');
            $flutterwaveEnabled = $request->has('flutterwave_status') && ($request->flutterwave_status == 1 || $request->flutterwave_status == '1');
    ...
    public function page() {
        ...
        return view('settings.' . $type_menu, compact('settings', 'type_menu', 'languages', 'stripe_currencies','languages_translate','translations'));
    }
        ...
        try {
            ...
            foreach ($fieldsToUpdate as $name) {
                ...
                if ($request->hasFile($name) && $name === 'why_choose_us_image') {
                    $file = $request->file($name);
                    $path = FileService::compressAndReplace($file, 'why-choose-us', $settings['why_choose_us_image'] ?? null);
                    $value = $path;
                    $type = 'file';
                } else {
    ...
    public function becomeInstructor()
    {
        ...
        if (\App\Services\InstructorModeService::isSingleInstructorMode()) {
            return redirect()->route('settings.system')->with('info', 'Become Instructor settings are disabled in Single Instructor mode.');
        }
        ...
        try{
            ...
            if ($request->hasFile('update_file')) {
                ...
                curl_setopt_array($curl, array(
                    CURLOPT_URL            => 'https://validator.wrteam.in/elms_validator?purchase_code=' . $request->purchase_code . '&domain_url=' . $app_url,
                ...
                if ($current_version != $version_file['current_version']) {
                    unlink($ver_file1);
                    unlink($source_path1);
                    return ResponseService::errorResponse($current_version . ' ' . trans('Please update nearest version'));
                }
                ...
                return ResponseService::successResponse('System updated successfully.', null, ['redirect_url' => route('settings.system-update')]);
            }
