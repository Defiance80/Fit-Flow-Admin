...
class FlutterwaveCheckoutService implements PaymentGatewayContract
{
    public function initiate(Order $order, array $options = []): array
    {
        ...
        if (empty($flutterwaveSettings['flutterwave_public_key']) || 
        ...
        if ($responseData['status'] !== 'success') {
            throw new \Exception('Flutterwave payment initialization failed: ' . ($responseData['message'] ?? 'Unknown error'));
        }
        ...
        if (!$link) {
            throw new \Exception('Flutterwave payment link not found in response. Response data: ' . json_encode($data));
        }
