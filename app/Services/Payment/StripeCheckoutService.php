...
class StripeCheckoutService implements PaymentGatewayContract
{
    public function initiate(Order $order, array $options = []): array
    {
        ...
        $successUrl =url('/stripe-callback') . '?session_id={CHECKOUT_SESSION_ID}&order=' . $order->order_number . '&type=' . $type;
        ...
        $zeroDecimal = in_array($currency, ['BIF','CLP','DJF','GNF','JPY','KMF','KRW','MGA','PYG','RWF','UGX','VND','VUV','XAF','XOF','XPF']);
