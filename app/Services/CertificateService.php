...
class CertificateService
{
    ...
    public function generateCourseCompletionCertificate($userId, $courseId, $certificateId = null)
    {
        try {
            ...
            if ($certificateId) {
                ...
                $certificate = Certificate::where('type', 'course_completion')
    ...
    private function createCertificateImage($certificate, $data)
    {
        ...
        if ($certificate->background_image) {
            ...
            $image->rectangle(0, 0, $width, $height, function ($draw) {
        ...
        if ($data['certificate_title']) {
            $image->text($data['certificate_title'], $width/2, 150, function($font) {
        ...
        if ($data['certificate_subtitle']) {
            $image->text($data['certificate_subtitle'], $width/2, 220, function($font) {
        ...
        // Add user name
        $image->text($data['user_name'], $width/2, 350, function($font) {
            $font->file(public_path('fonts/arial.ttf'));
            $font->size(36);
            $font->color('#ffffff');
            $font->align('center');
        ...
        $image->text('has successfully completed the course', $width/2, 420, function($font) {
            $font->file(public_path('fonts/arial.ttf'));
            $font->size(20);
            $font->color('#ffffff');
            $font->align('center');
        ...
        // Add course name
        $image->text($data['course_name'], $width/2, 480, function($font) {
            $font->file(public_path('fonts/arial.ttf'));
            $font->size(28);
            $font->color('#ffffff');
            $font->align('center');
        ...
        // Add completion date
        $image->text('on this day of ' . $data['completion_date'], $width/2, 540, function($font) {
            $font->file(public_path('fonts/arial.ttf'));
            $font->size(18);
            $font->color('#ffffff');
            $font->align('center');
        ...
        if ($data['signature_text']) {
            $image->text($data['signature_text'], $width - 200, $height - 80, function($font) {
                $font->file(public_path('fonts/arial.ttf'));
                $font->size(16);
                $font->color('#ffffff');
                $font->align('center');
        ...
        // Add date
        $image->text('Date: ' . $data['completion_date'], 100, $height - 50, function($font) {
            $font->file(public_path('fonts/arial.ttf'));
            $font->size(16);
            $font->color('#ffffff');
            $font->align('left');
