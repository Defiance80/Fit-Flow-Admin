...
class InstructorStatusUpdateNotification extends Notification implements ShouldQueue
{
     ...
     */
    public function __construct(Instructor $instructor, string $status, ?string $reason = null)
    {
    ...
    {
        $title = ucfirst($this->status) . ' - Instructor Application';
    ...
    {
        return match($this->status) {
            'approved' => 'Congratulations! Your instructor application has been approved. You can now start creating courses.',
    ...
    private function sendFcmNotification($notifiable)
    {
        try {
            ...
            $instructorTokens = UserFcmToken::where('user_id', $notifiable->id)
            ...
            foreach ($instructorTokens as $instructorToken) {
                try {
                    ...
                    Log::debug("Sending FCM instructor status update", [
                        'token' => substr($instructorToken->fcm_token, 0, 20) . '...',
                        'platform' => $platform,
                        'status' => $this->status
                    ]);
