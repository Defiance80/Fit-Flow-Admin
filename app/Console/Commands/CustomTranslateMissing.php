...
class CustomTranslateMissing extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($baseTranslations as $key => $englishValue) {
            $currentValue = isset($localeTranslations[$key]) ? trim($localeTranslations[$key]) : '';
        ...
        for ($batch = 0; $batch < $totalBatches; $batch++) {
            ...
            $this->info("Processing batch " . ($batch + 1) . " of " . $totalBatches . " (" . count($batchKeys) . " items)");
        ...
        $this->info("Translation completed successfully. {$translatedCount} strings translated and saved as {$fileName}.");
