...
Route::middleware('auth:sanctum')->group(function () {
    ...
    Route::group(['prefix' => 'promo-code'], function () {
        ...
        Route::post('/calculate-discount', [PromoCodeApiController::class, 'calculateCourseDiscount']);
    ...
    Route::group(['prefix' => 'assignments'], function () {
        Route::post('/submit', [\App\Http\Controllers\API\UserAssignmentSubmissionController::class, 'submitAssignment']);
        Route::get('/submissions', [\App\Http\Controllers\API\UserAssignmentSubmissionController::class, 'getUserSubmissions']);
        Route::get('/submission/{id}', [\App\Http\Controllers\API\UserAssignmentSubmissionController::class, 'getSubmissionDetails']);
        Route::get('/submission', [\App\Http\Controllers\API\UserAssignmentSubmissionController::class, 'getSubmissionDetails']); // Query parameter version
        Route::get('/course/{courseId}', [\App\Http\Controllers\API\UserAssignmentSubmissionController::class, 'getCourseAssignments']);
        Route::post('/submission/{id}', [\App\Http\Controllers\API\UserAssignmentSubmissionController::class, 'updateSubmission']);
        Route::post('/submission', [\App\Http\Controllers\API\UserAssignmentSubmissionController::class, 'updateSubmission']); // Query parameter version
        Route::delete('/submission', [\App\Http\Controllers\API\UserAssignmentSubmissionController::class, 'deleteSubmission']); // Query parameter version
        Route::delete('/submission/{id}', [\App\Http\Controllers\API\UserAssignmentSubmissionController::class, 'deleteSubmission']);
    });
     ...
     */
    Route::middleware([\App\Http\Middleware\CheckInstructorAccess::class])->group(function () {
        /**
    ...
    Route::put('/common/change-status', [Controller::class, 'changeStatus']);
        ...
        Route::get('earnings', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getInstructorEarnings']);
        Route::get('sales-statistics', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getInstructorSalesStatistics']);
        Route::get('withdrawal-details', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getWithdrawalDetails']);
        Route::get('withdrawal-history', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getWithdrawalHistory']);
        Route::post('request-withdrawal', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'requestWithdrawal']);
        Route::get('course-analysis', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getCourseAnalysis']);
    ...
    Route::group(['prefix' => 'course-chapters'], function () {
        ...
        Route::get('particular-curriculum/details', [CourseChaptersController::class, 'getParticularCurriculumDetails']);
        ...
        Route::delete('curriculum/quiz/delete-questions', [CourseChaptersController::class, 'quizQuestionsBulkDelete']);
    ...
    Route::put('admin/assignment-submissions/bulk-update', [AdminApiController::class, 'bulkUpdateAssignmentSubmissions']); // Bulk Update Assignment Submissions (Admin)
    ...
    */
    ...
    Route::prefix('finance')->group(function () {
        ...
        Route::post('admin/withdrawal-request/update-status', [FinanceApiController::class, 'updateWithdrawalRequestStatus']); // Admin
 ...
 */
Route::middleware('auth:sanctum')->group(function () {
    Route::get('instructor/earnings', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getInstructorEarnings']);
    Route::get('instructor/sales-statistics', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getInstructorSalesStatistics']);
    Route::get('instructor/withdrawal-details', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getWithdrawalDetails']);
    Route::get('instructor/withdrawal-history', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getWithdrawalHistory']);
    Route::post('instructor/request-withdrawal', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'requestWithdrawal']);
    Route::get('instructor/course-analysis', [\App\Http\Controllers\API\InstructorEarningsApiController::class, 'getCourseAnalysis']);
});
 ...
 * ===== FIT FLOW API =====
 */
use App\Http\Controllers\API\FitFlow\HealthMetricsApiController;
use App\Http\Controllers\API\FitFlow\ConnectionApiController;
use App\Http\Controllers\API\FitFlow\FitFlowDashboardApiController;

Route::prefix('fitflow')->middleware('auth:sanctum')->group(function () {

    // Trainer-Client Connection
    Route::post('connect', [ConnectionApiController::class, 'connect']);
    Route::post('disconnect', [ConnectionApiController::class, 'disconnect']);
    Route::get('connections', [ConnectionApiController::class, 'index']);
    Route::get('invite-code', [ConnectionApiController::class, 'getInviteCode']);

    // Health Metrics
    Route::prefix('health')->group(function () {
        Route::post('sync', [HealthMetricsApiController::class, 'sync']);
        Route::get('metrics', [HealthMetricsApiController::class, 'index']);
        Route::get('summary', [HealthMetricsApiController::class, 'summary']);
        Route::post('consent', [HealthMetricsApiController::class, 'updateConsent']);
        Route::get('alerts', [HealthMetricsApiController::class, 'alerts']);
        Route::post('alerts/{id}/acknowledge', [HealthMetricsApiController::class, 'acknowledgeAlert']);
    });

    // Dashboards
    Route::get('dashboard/client', [FitFlowDashboardApiController::class, 'clientDashboard']);
    Route::get('dashboard/trainer', [FitFlowDashboardApiController::class, 'trainerDashboard']);
});

use App\Http\Controllers\API\FitFlow\ProgramsApiController;
use App\Http\Controllers\API\FitFlow\ExercisesApiController;
use App\Http\Controllers\API\FitFlow\WorkoutLogApiController;
use App\Http\Controllers\API\FitFlow\ProgressApiController;
use App\Http\Controllers\API\FitFlow\ScheduleApiController;
use App\Http\Controllers\API\FitFlow\MealPlanApiController;
use App\Http\Controllers\API\FitFlow\OnboardingApiController;

// Public registration (no auth)
Route::prefix('fitflow')->group(function () {
    Route::post('register/trainer', [OnboardingApiController::class, 'registerTrainer']);
    Route::post('register/client', [OnboardingApiController::class, 'registerClient']);
});

// Authenticated Fit Flow APIs
Route::prefix('fitflow')->middleware('auth:sanctum')->group(function () {
    // Profile
    Route::get('profile', [OnboardingApiController::class, 'profile']);
    Route::put('profile', [OnboardingApiController::class, 'updateProfile']);
    Route::post('profile', [OnboardingApiController::class, 'updateProfile']); // for multipart

    // Training Programs
    Route::apiResource('programs', ProgramsApiController::class);
    Route::post('programs/assign', [ProgramsApiController::class, 'assignToClient']);

    // Exercise Library
    Route::apiResource('exercises', ExercisesApiController::class)->parameters(['exercises' => 'exercise']);
    Route::get('exercise-categories', [ExercisesApiController::class, 'categories']);

    // Workout Logging
    Route::get('workout-logs', [WorkoutLogApiController::class, 'index']);
    Route::post('workout-logs', [WorkoutLogApiController::class, 'store']);
    Route::get('workout-logs/{id}', [WorkoutLogApiController::class, 'show']);

    // Progress Tracking
    Route::get('measurements', [ProgressApiController::class, 'getMeasurements']);
    Route::post('measurements', [ProgressApiController::class, 'storeMeasurement']);
    Route::get('progress-photos', [ProgressApiController::class, 'getPhotos']);
    Route::post('progress-photos', [ProgressApiController::class, 'storePhoto']);
    Route::get('progress-summary', [ProgressApiController::class, 'getProgressSummary']);

    // Scheduling
    Route::apiResource('schedules', ScheduleApiController::class)->parameters(['schedules' => 'schedule']);
    Route::get('upcoming-sessions', [ScheduleApiController::class, 'upcoming']);

    // Meal Plans
    Route::apiResource('meal-plans', MealPlanApiController::class)->parameters(['meal-plans' => 'mealPlan']);
});
